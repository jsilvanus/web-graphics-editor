...
export class ThreeGraphics3DRenderer implements Graphics3DRenderer {
  ...
  render(
    ...
    const backgroundOpacity = Math.max(0, Math.min(1, options.backgroundOpacity ?? 1));
    if (options.background !== undefined) scene.background = new THREE.Color(options.background);
    if (!this.renderer) return scene;
    const width = Math.max(1, options.width ?? this.container?.clientWidth ?? 1),
      height = Math.max(1, options.height ?? this.container?.clientHeight ?? 1),
      pixelRatio = Math.max(0.25, options.pixelRatio ?? Math.min(window.devicePixelRatio || 1, 2));
    ...
    this.renderer.setClearColor(
      ...
      options.background === undefined ? 0 : backgroundOpacity,
    );
