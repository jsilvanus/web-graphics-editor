...
/**
 * Editor-owned boundary around Three.js.
 *
 * Three.js objects are deliberately created here and never persisted in the
 * graphics document. 3D rotations are stored in radians.
 */
...
function materialFor(material?: Graphics3DMaterial): THREE.MeshStandardMaterial {
  const value = material ?? {};
  return new THREE.MeshStandardMaterial({
    color: value.color ?? "#ffffff",
    opacity: value.opacity ?? 1,
    transparent: (value.opacity ?? 1) < 1,
...
function visible(mesh: Graphics3DMesh, view?: Pick<Graphics3DView, "visibility">): boolean {
...
function addLight(scene: THREE.Scene, light: Graphics3DLight): void {
  ...
  switch (light.type) {
    ...
    case "point":
      ...
      break;
    case "spot":
      object = new THREE.SpotLight(color, intensity, light.distance ?? 0, light.angle ?? Math.PI / 3, light.penumbra ?? 0);
      break;
  }
...
export function createThreeScene(world: Graphics3DWorld, view?: Pick<Graphics3DView, "visibility">): THREE.Scene {
  ...
  for (const mesh of world.meshes) {
    if (!visible(mesh, view)) continue;
    const object = new THREE.Mesh(createThreeGeometry(mesh), materialFor(mesh.material));
