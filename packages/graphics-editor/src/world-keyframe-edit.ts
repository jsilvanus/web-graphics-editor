...
export function editWorldPropertyAtTime(
  ...
  const base = existing
    ...
    : {
        ...
        timeline: {
          ...(world.timeline ?? {}),
          tracks: [...tracks, { id: trackId, targetType, targetId, property, keyframes: [] }],
        },
      };
  const next = upsert3DKeyframe(base, trackId, k);
  return { world: next, trackId, keyframeId: k.id, created: !keyframe };
}
