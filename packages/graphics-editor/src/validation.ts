...
export function validateGraphicsDocument(document: unknown): DocumentValidationResult {
  ...
  if (!Number.isFinite(d.width) || d.width! <= 0) errors.push("Document width must be a positive finite number");
  if (!Number.isFinite(d.height) || d.height! <= 0) errors.push("Document height must be a positive finite number");
  ...
  for (const layer of d.layers) {
    ...
    if (!["text","image","rectangle","ellipse","line","path","group","3d-view"].includes(layer.type)) errors.push(`Invalid layer type: ${String(layer.type)}`);
    for (const key of ["x","y","width","height"]) if (!Number.isFinite(layer[key as keyof typeof layer] as number)) errors.push(`Layer ${layer.id ?? "?"} has invalid ${key}`);
  }
  for (const layer of d.layers) {
    if (layer.parentId && !ids.has(layer.parentId)) errors.push(`Layer ${layer.id} references missing parent ${layer.parentId}`);
  }
  for (const layer of d.layers) {
    const seen = new Set<string>();
    let current = layer;
    while (current.parentId) {
      if (seen.has(current.id)) { errors.push(`Layer parent cycle detected at ${layer.id}`); break; }
      seen.add(current.id);
