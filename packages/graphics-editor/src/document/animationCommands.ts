...
export interface AnimationCommandResult {
...
export function setAnimatedPropertyAtTime(
      ...
      document: {
        ...
        timeline: {
          ...
          tracks: timeline.tracks.map(item =>
            item.id === track.id ? upsertKeyframe(item, { ...existing, value }) : item,
          ),
        },
      },
      operation,
  ...
  const nextTimeline = track
    ? {
        ...timeline,
        tracks: timeline.tracks.map(item => (item.id === track.id ? upsertKeyframe(item, keyframe) : item)),
      }
    : { ...timeline, tracks: [...timeline.tracks, upsertKeyframe(nextTrack, keyframe)] };
  return {
    document: { ...document, timeline: nextTimeline },
    operation: { type: "add-keyframe", track: nextTrack, keyframe },
  };
}

export function enableAnimatedProperty(
  document: GraphicsDocument,
  layerId: string,
  property: AnimatedProperty,
  time: number,
...
export function setCompositionAnimatedPropertyAtTime(
  ...
  const keyframe = {
    id:
      track?.keyframes.find(item => Math.abs(item.time - time) < 0.0001)?.id ??
    ...
    ? {
        ...
        keyframes: [
          ...track.keyframes.filter(
            item => item.id !== keyframe.id && Math.abs(item.time - keyframe.time) > 0.0001,
          ),
          keyframe,
  ...
  return {
    ...document,
    compositions: document.compositions?.map(item =>
      item.id === compositionId
        ? {
            ...
            timeline: {
              ...timeline,
              tracks: track
                ? timeline.tracks.map(existing => (existing.id === track.id ? nextTrack : existing))
                : [...timeline.tracks, nextTrack],
            },
          }
        : item,
    ),
