...
export function updateLayerStyleCommand(
  document: GraphicsDocument,
  id: string,
  key: string,
  ...
  if (value === undefined) {
    ...
    delete style[key];
    return {
      document: updateLayer(document, id, { style }),
      operation: { type: "set-layer-style", layerId: id, property: key, from, to: value },
    };
  }
  return {
    document: updateLayerStyle(document, id, key, value),
    operation: { type: "set-layer-style", layerId: id, property: key, from, to: value },
  };
}
...
export function reorderLayerCommand(
  ...
  const targetIndex =
    action === "forward"
      ? Math.min(index + 1, document.layers.length - 1)
      : action === "backward"
        ? Math.max(index - 1, 0)
        : action === "front"
  ...
  const next =
    action === "forward"
      ? bringLayerForward(document, id)
      : action === "backward"
        ? sendLayerBackward(document, id)
        : action === "front"
          ? bringLayerToFront(document, id)
          : sendLayerToBack(document, id);
  return {
    document: next,
    operation: { type: "reorder-layer", layerId: id, fromIndex: index, toIndex: targetIndex },
  };
}
...
export function groupLayersCommand(document: GraphicsDocument, ids: Set<string>): CommandResult {
  ...
  const children = childSnapshots(
    document,
    [...ids].filter(id => document.layers.some(layer => layer.id === id && layer.type !== "group")),
  );
...
export function ungroupLayerCommand(document: GraphicsDocument, id: string): CommandResult {
  ...
  const children = childSnapshots(document, group.children);
  const index = document.layers.findIndex(layer => layer.id === id);
  return {
    document: ungroupLayer(document, id),
    operation: { type: "ungroup-layer", group: { ...group }, children, index },
  };
}
...
export function removeLayerCommand(document: GraphicsDocument, id: string): CommandResult {
  ...
  return {
    ...
    operation: { type: "remove-layer", layer: document.layers[index], index },
  };
}
...
export function alignLayersCommand(
  document: GraphicsDocument,
  ids: Set<string>,
  mode: AlignMode,
...
export function distributeLayersCommand(
  document: GraphicsDocument,
  ids: Set<string>,
  mode: DistributeMode,
): CommandResult {
...
function layerPolygon(layer: Layer): PolygonPoint[] | null {
  ...
  if (layer.type === "ellipse") {
    return Array.from({ length: 64 }, (_, i) => {
      ...
      return {
        x: layer.x + layer.width / 2 + (Math.cos(a) * layer.width) / 2,
        y: layer.y + layer.height / 2 + (Math.sin(a) * layer.height) / 2,
      };
    });
  }
  return null;
...
export function moveLayerCommand(
  ...
  const visit = (layerId: string) => {
    if (descendants.has(layerId)) return;
  ...
  const parent = parentId
    ? document.layers.find(layer => layer.id === parentId && layer.type === "group")
    : undefined;
  ...
  let next = {
    ...
    layers: document.layers.map(layer => ({
      ...layer,
      children: layer.children ? [...layer.children] : layer.children,
    })),
  };
  ...
  if (container?.children) {
    ...
    if (position === "inside") container.children.push(id);
      ...
      const index = container.children.indexOf(targetId);
      container.children.splice(Math.max(0, position === "before" ? index : index + 1), 0, id);
    }
  } else if (!parentId) {
...
export function booleanLayersCommand(
  document: GraphicsDocument,
  ids: string[],
  ...
  const a = document.layers.find(layer => layer.id === ids[0]),
    b = document.layers.find(layer => layer.id === ids[1]);
  if (!a || !b || a.locked || b.locked) return { document };
  const pa = layerPolygon(a),
    pb = layerPolygon(b);
  ...
  const baseStyle = {
    ...(a.style ?? {}),
    "fill-rule": contours.some(contour => contour.hole)
      ? "evenodd"
      : String(a.style?.["fill-rule"] ?? "nonzero"),
  };
  const copy = {
    ...a,
    id: `boolean-${operation}-${Date.now()}-${Math.random().toString(36).slice(2, 5)}`,
    type: "path" as const,
    x: minX,
    y: minY,
  ...
  const next = {
    ...document,
    layers: document.layers.filter(layer => layer.id !== a.id && layer.id !== b.id).concat(copies),
  };
  return { document: next, operation: batchOrSingle(diffOperations(document, next)) };
}
...
export function joinPathLayersCommand(document: GraphicsDocument, ids: string[]): CommandResult {
  ...
  const aWorld = a.nodes.map(n => ({
    ...n,
    x: n.x + a.x,
    y: n.y + a.y,
    handleIn: n.handleIn && { x: n.handleIn.x + a.x, y: n.handleIn.y + a.y },
    handleOut: n.handleOut && { x: n.handleOut.x + a.x, y: n.handleOut.y + a.y },
  }));
  const bWorld = b.nodes.map(n => ({
    ...n,
    x: n.x + b.x,
    y: n.y + b.y,
    handleIn: n.handleIn && { x: n.handleIn.x + b.x, y: n.handleIn.y + b.y },
    handleOut: n.handleOut && { x: n.handleOut.x + b.x, y: n.handleOut.y + b.y },
  }));
  const all = [...aWorld, ...bWorld];
  const minX = Math.min(...all.map(n => n.x)),
    minY = Math.min(...all.map(n => n.y)),
    maxX = Math.max(...all.map(n => n.x)),
    maxY = Math.max(...all.map(n => n.y));
  const nodes = all.map(n => ({
    ...n,
    x: n.x - minX,
    y: n.y - minY,
    handleIn: n.handleIn && { x: n.handleIn.x - minX, y: n.handleIn.y - minY },
    handleOut: n.handleOut && { x: n.handleOut.x - minX, y: n.handleOut.y - minY },
  }));
  const next = {
    ...document,
    layers: document.layers
      .filter(layer => layer.id !== b.id)
      .map(layer =>
        layer.id === a.id
          ? {
              ...layer,
...
export function offsetPathCommand(document: GraphicsDocument, id: string, distance: number): CommandResult {
  const layer = document.layers.find(l => l.id === id);
  if (!layer || layer.type !== "path" || !layer.nodes?.length || !Number.isFinite(distance) || distance === 0)
    return { document };
  const nodes = offsetPathNodes(layer.nodes, distance, !!layer.closed);
  const xs = nodes.map(n => n.x),
    ...
    maxX = Math.max(...xs),
    maxY = Math.max(...ys);
  const normalized = nodes.map(n => ({ ...n, x: n.x - minX, y: n.y - minY }));
  const next = updateLayer(document, id, {
    x: layer.x + minX,
    y: layer.y + minY,
    width: Math.max(1, maxX - minX),
    height: Math.max(1, maxY - minY),
...
export function flattenPathCommand(document: GraphicsDocument, id: string, tolerance = 0.75): CommandResult {
  ...
  const xs = flattened.points.map(p => p.x),
    ys = flattened.points.map(p => p.y);
...
export function convertLayerToPathCommand(document: GraphicsDocument, id: string): CommandResult {
  ...
  if (layer.type === "rectangle")
    nodes = [
      { x: 0, y: 0, kind: "corner" },
      { x: layer.width, y: 0, kind: "corner" },
      { x: layer.width, y: layer.height, kind: "corner" },
      { x: 0, y: layer.height, kind: "corner" },
    ];
  else {
    ...
    nodes = [
      {
        ...
        y: 0,
        kind: "smooth",
        handleIn: { x: cx - k * rx, y: 0 },
        handleOut: { x: cx + k * rx, y: 0 },
      },
      {
        x: layer.width,
        y: cy,
        kind: "smooth",
        handleIn: { x: layer.width, y: cy - k * ry },
        handleOut: { x: layer.width, y: cy + k * ry },
      },
      {
        x: cx,
        y: layer.height,
        kind: "smooth",
        handleIn: { x: cx + k * rx, y: layer.height },
        handleOut: { x: cx - k * rx, y: layer.height },
      },
      {
        x: 0,
        y: cy,
        kind: "smooth",
        handleIn: { x: 0, y: cy + k * ry },
        handleOut: { x: 0, y: cy - k * ry },
      },
    ];
  }
  const next = updateLayer(document, id, {
...
export function convertPathToShapeCommand(document: GraphicsDocument, id: string): CommandResult {
  ...
  const ellipseLike =
    nodes.length === 4 && nodes.every(n => n.kind === "smooth" && n.handleIn && n.handleOut);
  if (!rectangleLike && !ellipseLike) return { document };
  const next = updateLayer(document, id, {
    type: ellipseLike ? "ellipse" : "rectangle",
    x: layer.x + minX,
    y: layer.y + minY,
    width: Math.max(1, maxX - minX),
    height: Math.max(1, maxY - minY),
