...
export function setTimelineCommand(document: GraphicsDocument, timeline: SceneTimeline | undefined, label = "Update timeline"): TimelineCommandResult {
...
export function updateTimelineCommand(document: GraphicsDocument, updater: (timeline: SceneTimeline) => SceneTimeline, fallback: SceneTimeline, label = "Update timeline"): TimelineCommandResult {
  const current = document.timeline ?? fallback;
  return setTimelineCommand(document, updater(current), label);
}
