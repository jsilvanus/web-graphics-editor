...
/** Weld selected vertices to nearby vertices, or all vertices when no selection is supplied. */
export function weldVertices(mesh: Graphics3DMesh, vertexIds?: number[], tolerance = 1e-6): Graphics3DMesh {
  ...
  const selected = vertexIds
    ? new Set(vertexIds.filter(id => Number.isInteger(id) && id >= 0 && id < count))
    : null;
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (let j = 0; j < i; j++) {
      const b: Vec3 = [
        mesh.geometry.vertices[j * 3],
        mesh.geometry.vertices[j * 3 + 1],
        mesh.geometry.vertices[j * 3 + 2],
      ];
  ...
  return {
    ...mesh,
    geometry: { ...mesh.geometry, vertices, indices: mesh.geometry.indices.map(index => indexMap[index]) },
  };
}
