...
const vertex = (g: Graphics3DMesh["geometry"], index: number): Vec3 => [
  g.vertices[index * 3],
  g.vertices[index * 3 + 1],
  g.vertices[index * 3 + 2],
];
const addVertex = (vertices: number[], v: Vec3) => {
...
const normalize = (v: Vec3): Vec3 => {
  const n = length(v);
  return n > 1e-8 ? [v[0] / n, v[1] / n, v[2] / n] : [0, 1, 0];
};
...
export function faceNormal(mesh: Graphics3DMesh, faceIndex: number): Vec3 | null {
  const ids = faceVertexIndices(mesh, faceIndex);
  if (!ids) return null;
  return normalize(
    cross(
      sub(vertex(mesh.geometry, ids[1]), vertex(mesh.geometry, ids[0])),
      sub(vertex(mesh.geometry, ids[2]), vertex(mesh.geometry, ids[0])),
    ),
  );
}
...
export function extrudeFace(mesh: Graphics3DMesh, faceIndex: number, distance: number): Graphics3DMesh {
  ...
  if (!ids || !normal) return mesh;
  const vertices = [...mesh.geometry.vertices];
  const a = vertex(mesh.geometry, ids[0]),
    b = vertex(mesh.geometry, ids[1]),
    c = vertex(mesh.geometry, ids[2]);
