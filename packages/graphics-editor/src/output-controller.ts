...
export interface OutputControllerOptions {
  ...
  updateOutput?: (
    outputId: string,
    changes: Partial<Pick<GraphicsOutput, "playback" | "background">>,
...
export class OutputController {
  ...
  initialize(outputId: string): OutputRuntime | undefined {
    ...
    const initial = dispatchOutputRuntime(
      {
        outputId,
        state: "off",
  ...
  dispatch(command: OutputCommand): OutputEvent[] {
    ...
    if ((command.type === "output.take" || command.type === "output.takeOff") && !output.liveControl)
      return [this.error("not_allowed", "Live control is disabled for this output", command)];
    let runtime = this.runtimes.get(command.outputId) ?? this.initialize(command.outputId)!;
    let events: OutputEvent[] = [];
    switch (command.type) {
      case "output.take":
        runtime = dispatchOutputRuntime(runtime, output, { type: "TAKE" });
        break;
      case "output.takeOff":
        runtime = dispatchOutputRuntime(runtime, output, { type: "TAKE_OFF" });
        break;
      case "output.play":
        runtime = dispatchOutputRuntime(runtime, output, { type: "PLAY" });
        break;
      case "output.pause":
        runtime = dispatchOutputRuntime(runtime, output, { type: "PAUSE" });
        break;
      case "output.seek":
        runtime = dispatchOutputRuntime(runtime, output, { type: "SEEK", time: command.time });
        break;
      case "output.reset":
        runtime = dispatchOutputRuntime(runtime, output, { type: "RESET" });
        break;
      case "output.setPlayback": {
        ...
        if (!updated)
          return [
            this.error(
              "not_allowed",
              "Persistent output configuration cannot be changed by this controller",
              command,
            ),
          ];
  ...
  private error(
    code: "not_found" | "not_allowed" | "invalid",
    message: string,
  ...
  private emit(events: OutputEvent[]) {
