...
export function moveMeshVertices(
  ...
  for (const id of selected) {
    ...
    vertices[offset] += delta[0];
    vertices[offset + 1] += delta[1];
    vertices[offset + 2] += delta[2];
  }
...
export function deleteMeshFaces(mesh: Graphics3DMesh, faceIds: number[]): Graphics3DMesh {
  ...
  const kept: number[] = [];
  for (let face = 0; face < oldIndices.length / 3; face++)
    if (!removed.has(face))
      kept.push(oldIndices[face * 3], oldIndices[face * 3 + 1], oldIndices[face * 3 + 2]);
...
export function mergeMeshVertices(mesh: Graphics3DMesh, tolerance = 1e-6): Graphics3DMesh {
  ...
  for (let i = 0; i < map.length; i++) {
    ...
    for (let j = 0; j < compact.length / 3; j++) {
      if (Math.hypot(x - compact[j * 3], y - compact[j * 3 + 1], z - compact[j * 3 + 2]) <= limit) {
        found = j;
        break;
      }
    }
    if (found < 0) {
      found = compact.length / 3;
      compact.push(x, y, z);
    }
    map[i] = found;
  }
  return {
    ...mesh,
    geometry: { vertices: compact, indices: mesh.geometry.indices.map(index => map[index]) },
  };
}
