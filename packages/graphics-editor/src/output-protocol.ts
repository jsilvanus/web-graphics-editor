...
export function isOutputCommand(value: unknown): value is OutputCommand {
  ...
  switch (v.type) {
    ...
    case "output.reset":
      return true;
    case "output.seek":
      return typeof v.time === "number" && Number.isFinite(v.time);
    case "output.setPlayback":
      return ["static", "automatic", "user", "live"].includes(v.playback as string);
    case "output.setBackground":
      return v.background === "transparent" || v.background === "opaque";
    default:
      return false;
  }
}
