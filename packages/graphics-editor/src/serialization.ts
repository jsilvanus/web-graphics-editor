...
export function deserializeGraphicsDocument(input: string | SerializedDocument): GraphicsDocument {
  const value: SerializedDocument =
    typeof input === "string" ? (JSON.parse(input) as SerializedDocument) : input;
  ...
  const document = {
    ...
    layers: value.layers.map(layer => ({ ...layer, type: layer.type === "rect" ? "rectangle" : layer.type })),
