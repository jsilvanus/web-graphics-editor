...
const base: GraphicsDocument = {
  ...
  layers: [
    {
      ...
      opacity: 1,
      visible: true,
      viewportOverrides: { v: { x: 50, opacity: 0.5 } },
    },
  ],
  ...
  timeline: {
    scenes: [
      { id: "s1", name: "S1", compositionId: "c", start: 0, duration: 2 },
      { id: "s2", name: "S2", compositionId: "c", start: 2, duration: 3 },
    ],
    currentSceneId: "s1",
    currentTime: 0,
    tracks: [],
  ...
  outputs: [{ id: "o", name: "O", viewportId: "v", playback: "static", background: "transparent" }],
};
...
describe("presentation edge cases", () => {
  ...
  it("does not mutate nested viewport override objects", () => {
    const before = JSON.stringify(base);
    resolveViewportComposition(base, "v");
    expect(JSON.stringify(base)).toBe(before);
  });
  it("uses scene boundaries consistently", () => {
    expect(sceneAtTime(base.timeline, 0)?.id).toBe("s1");
    expect(sceneAtTime(base.timeline, 2)?.id).toBe("s2");
  ...
  it("returns local scene time from the selected scene", () => {
    const r = resolveScene(base, 3, "v")!;
    expect(r.scene.id).toBe("s2");
    expect(r.localTime).toBe(1);
  });
});
