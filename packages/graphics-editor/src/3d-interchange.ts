...
/** Export one world to glTF/GLB. Editor-specific data is carried in node extras where possible. */
export async function export3DWorld(world: Graphics3DWorld, binary = true): Promise<Graphics3DInterchangeResult> {
  const scene = new THREE.Scene(); scene.name = world.name ?? world.id;
  ...
  for (const light of world.lights ?? []) {
    let object: THREE.Light; const color = light.color ?? "#ffffff", intensity = light.intensity ?? 1;
    ...
    object.name = light.id; object.userData.graphics3DId = light.id; if (light.position) object.position.set(...light.position); if (light.rotation) object.rotation.set(...light.rotation); scene.add(object);
  }
  for (const camera of world.cameras) {
    ...
    object.name = camera.name ?? camera.id; object.userData.graphics3DId = camera.id; if (camera.projection === "orthographic") object.zoom = camera.zoom ?? 1; object.position.set(...camera.position); object.rotation.set(...camera.rotation); scene.add(object);
  }
  const warnings: string[] = [];
  if (world.provenance) warnings.push("World provenance is editor metadata; it is not guaranteed to survive all glTF consumers.");
  if (world.meshes.some(mesh => mesh.material?.textureAssetId)) warnings.push("Texture asset IDs require host-side asset resolution; GLB export currently exports geometry/material properties, not editor asset URLs.");
...
function readMesh(object: THREE.Mesh): Graphics3DMesh {
  ...
  const material = Array.isArray(object.material) ? object.material[0] : object.material; const standard = material as THREE.MeshStandardMaterial;
  return { id: String(object.userData.graphics3DId ?? object.uuid), name: object.name || undefined, geometry: { vertices, indices: Array.from({ length: vertices.length / 3 }, (_, i) => i), normals: normalAttr ? normals : undefined, uv: uvAttr ? uv : undefined }, transform: { position: object.position.toArray(), rotation: [object.rotation.x, object.rotation.y, object.rotation.z], scale: object.scale.toArray() }, material: { color: standard?.color?.getHexString() ? `#${standard.color.getHexString()}` : undefined, opacity: standard?.opacity, metalness: standard?.metalness, roughness: standard?.roughness, wireframe: standard?.wireframe }, provenance: object.userData.wegraProvenance as Provenance | undefined };
}
...
export async function import3DWorld(data: ArrayBuffer, id: string, name?: string): Promise<Graphics3DWorld> {
  const loader = new GLTFLoader(); const gltf = await new Promise<any>((resolve, reject) => loader.parse(data, "", resolve, reject));
  ...
  gltf.scene.traverse((object: THREE.Object3D) => {
    ...
    if (object instanceof THREE.AmbientLight || object instanceof THREE.DirectionalLight || object instanceof THREE.PointLight || object instanceof THREE.SpotLight) { const light = object as THREE.Light & { distance?: number; angle?: number; penumbra?: number }; const type = object instanceof THREE.AmbientLight ? "ambient" : object instanceof THREE.DirectionalLight ? "directional" : object instanceof THREE.PointLight ? "point" : "spot"; lights.push({ id: String(object.userData.graphics3DId ?? object.uuid), type, position: object.position.toArray(), rotation: [object.rotation.x, object.rotation.y, object.rotation.z], color: `#${object.color.getHexString()}`, intensity: object.intensity, distance: light.distance, angle: light.angle, penumbra: light.penumbra }); }
  });
