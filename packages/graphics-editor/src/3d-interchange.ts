...
export async function export3DWorld(
  ...
  for (const light of world.lights ?? []) {
    ...
    switch (light.type) {
      ...
      case "directional":
        ...
        break;
      case "point":
        object = new THREE.PointLight(color, intensity, light.distance ?? 0);
        break;
      case "spot":
  ...
  for (const camera of world.cameras) {
    const object =
      camera.projection === "orthographic"
        ? new THREE.OrthographicCamera(-5, 5, 5, -5, camera.near ?? 0.1, camera.far ?? 2000)
        : new THREE.PerspectiveCamera(camera.fov ?? 50, 1, camera.near ?? 0.1, camera.far ?? 2000);
    object.name = camera.name ?? camera.id;
    object.userData.graphics3DId = camera.id;
    if (camera.projection === "orthographic") object.zoom = camera.zoom ?? 1;
  ...
  if (world.provenance)
    warnings.push("World provenance is editor metadata; it is not guaranteed to survive all glTF consumers.");
  if (world.meshes.some(mesh => mesh.material?.textureAssetId))
    warnings.push(
      "Texture asset IDs require host-side asset resolution; GLB export currently exports geometry/material properties, not editor asset URLs.",
    );
  const exporter = new GLTFExporter();
  const data = await new Promise<ArrayBuffer | object>((resolve, reject) =>
    exporter.parse(scene, result => resolve(result as ArrayBuffer | object), reject, {
      binary,
      animations: [],
    }),
  );
...
function vec3(attribute: THREE.BufferAttribute, i: number): [number, number, number] {
...
function readMesh(object: THREE.Mesh): Graphics3DMesh {
  ...
  for (let i = 0; i < position.count; i++) {
    const p = vec3(position, i);
    vertices.push(...p);
    if (normalAttr) normals.push(...vec3(normalAttr, i));
    if (uvAttr) uv.push(uvAttr.getX(i), uvAttr.getY(i));
  }
  const material = Array.isArray(object.material) ? object.material[0] : object.material;
  const standard = material as THREE.MeshStandardMaterial;
  return {
    id: String(object.userData.graphics3DId ?? object.uuid),
    name: object.name || undefined,
    geometry: {
      vertices,
      indices: Array.from({ length: vertices.length / 3 }, (_, i) => i),
      normals: normalAttr ? normals : undefined,
      uv: uvAttr ? uv : undefined,
    },
    transform: {
    ...
    material: {
      color: standard?.color?.getHexString() ? `#${standard.color.getHexString()}` : undefined,
      opacity: standard?.opacity,
      metalness: standard?.metalness,
      roughness: standard?.roughness,
      wireframe: standard?.wireframe,
    },
    provenance: object.userData.wegraProvenance as Provenance | undefined,
  };
}
