...
import { createKeyframe, createTrack, createClip } from "../timeline";
...
const baseTimeline = (): SceneTimeline => ({
  scenes: [{ id: "scene", name: "Scene", start: 0, duration: 10 }],
...
describe("DocumentOperation", () => {
  ...
  it("reorders a layer and restores its position", () => {
    ...
    expect(roundTrip(document(layer("a"), layer("b"), layer("c")), operation).layers.map(x => x.id)).toEqual([
      "b",
      "c",
      "a",
    ]);
  ...
  it("round-trips transform, property and style changes", () => {
    ...
    const operation = {
      ...
      operations: [
        ...
        {
          type: "resize-layer" as const,
          layerId: "a",
          from: { x: 10, y: 20, width: 100, height: 100 },
          to: { x: 30, y: 40, width: 200, height: 150 },
        },
    ...
    expect(roundTrip(before, operation).layers[0]).toMatchObject({
      x: 30,
      y: 40,
      width: 200,
      height: 150,
  ...
  it("restores interleaved layer order when undoing a group", () => {
    const a = { ...layer("a"), parentId: undefined },
      b = { ...layer("b"), parentId: undefined },
      c = { ...layer("c"), parentId: undefined };
    const group: Layer = {
      id: "g",
      type: "group",
      x: 0,
  ...
  it("creates a keyframe at the exact playhead when a property is already animated", () => {
    const a = layer("a", 100);
    const track = createTrack("a", "x");
    const timeline = {
      ...baseTimeline(),
      tracks: [{ ...track, keyframes: [createKeyframe(0, 100), createKeyframe(10, 500)] }],
    };
    const before = { ...document(a), timeline };
    const result = setAnimatedPropertyAtTime(before, "a", "x", 3.25, 240);
    expect(result.operation?.type).toBe("add-keyframe");
    expect(
      result.document.timeline?.tracks[0].keyframes.find(k => Math.abs(k.time - 3.25) < 0.0001)?.value,
    ).toBe(240);
  });
  it("updates an existing keyframe instead of creating a duplicate", () => {
    const a = layer("a", 100);
    const track = createTrack("a", "x");
    const existing = createKeyframe(3.25, 200);
    const before = {
      ...document(a),
      timeline: { ...baseTimeline(), tracks: [{ ...track, keyframes: [existing] }] },
    };
    const result = setAnimatedPropertyAtTime(before, "a", "x", 3.25, 300);
    expect(result.operation?.type).toBe("update-keyframe");
    expect(result.document.timeline?.tracks[0].keyframes).toHaveLength(1);
    expect(result.document.timeline?.tracks[0].keyframes[0].value).toBe(300);
    expect(roundTrip(before, result.operation!)).toEqual(result.document);
  });
  it("moves a temporal clip without changing z-order", () => {
    const a = layer("a"),
      b = layer("b");
    const clip = createClip("a", 0, 2);
    const before = { ...document(a, b), timeline: { ...baseTimeline(), clips: [clip] } };
    const operation = {
      type: "set-clip-timing" as const,
      clipId: clip.id,
      from: { start: 0, duration: 2 },
      to: { start: 4, duration: 3 },
    };
    const after = roundTrip(before, operation);
    expect(after.layers.map(x => x.id)).toEqual(["a", "b"]);
    expect(after.timeline?.clips?.[0]).toMatchObject({ start: 4, duration: 3 });
  });
});
