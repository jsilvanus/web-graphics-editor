import type { Graphics3DCamera, Graphics3DLight, Graphics3DMaterial, Graphics3DMesh, Graphics3DTrack, Graphics3DWorld, Keyframe } from "../types";
...
function worldTracks(world:Graphics3DWorld):Graphics3DTrack[]{return world.timeline?.tracks??[]}
function withTracks(world:Graphics3DWorld,value:Graphics3DTrack[]):Graphics3DWorld{return{...world,timeline:{tracks:value}}}
function updateTrack(world:Graphics3DWorld,id:string,fn:(track:Graphics3DTrack)=>Graphics3DTrack):Graphics3DWorld{return withTracks(world,worldTracks(world).map(track=>track.id===id?fn(track):track))}
...
export function applyWorldOperation(world:Graphics3DWorld,operation:WorldOperation,reverse=false):Graphics3DWorld{
 ...
 if(operation.type==="add-3d-track")return withTracks(world,reverse?remove(worldTracks(world),operation.track.id):insert(worldTracks(world),operation.track,operation.index));
 if(operation.type==="remove-3d-track")return withTracks(world,reverse?insert(worldTracks(world),operation.track,operation.index):remove(worldTracks(world),operation.track.id));
 if(operation.type==="update-3d-track")return withTracks(world,updateById(worldTracks(world),operation.trackId,reverse?operation.from:operation.to));
...
export function invertWorldOperation(operation:WorldOperation):WorldOperation{
 ...
 case "add-3d-track":return{type:"remove-3d-track",track:operation.track,index:operation.index??0};case "remove-3d-track":return{type:"add-3d-track",track:operation.track,index:operation.index};case "update-3d-track":return{...operation,from:operation.to,to:operation.from};
 case "add-3d-keyframe":return{type:"remove-3d-keyframe",trackId:operation.trackId,keyframe:operation.keyframe,index:operation.index??0};case "remove-3d-keyframe":return{type:"add-3d-keyframe",trackId:operation.trackId,keyframe:operation.keyframe,index:operation.index};case "update-3d-keyframe":return{...operation,from:operation.to,to:operation.from};case "move-3d-keyframe":return{...operation,fromTime:operation.toTime,toTime:operation.fromTime};
 }
}
