...
const history: WorldHistory = {
  worldId: "world-1",
  entries: [
    { id: "world-op-1", timestamp: 1, label: "Move mesh", actor: "human", operation: worldOperation },
  ],
};
const entry: HistoryEntry = {
  id: "project-op-1",
  timestamp: 2,
  label: "Move mesh in world",
  actor: "human",
  operation: { type: "world-operation", worldId: "world-1", operationId: "world-op-1" },
};
...
describe("world history resolver", () => {
  ...
  it("undoes the referenced world operation", () => {
    const changed = resolveHistoryEntry(
      document,
      entry,
      new Map([[world.id, world]]),
      createWorldHistoryStore([history]),
    );
  ...
  it("fails clearly for missing world or operation", () => {
    expect(() => resolveHistoryEntry(document, entry, new Map(), createWorldHistoryStore([]))).toThrow(
      "3D world not found",
    );
    expect(() =>
      resolveHistoryEntry(
        ...
        entry,
        new Map([[world.id, world]]),
        createWorldHistoryStore([{ worldId: world.id, entries: [] }]),
      ),
    ).toThrow("3D world operation not found");
  });
});
