...
export function appendHistory(
  ...
  const checkpoints =
    entries.length % checkpointInterval === 0
      ? [
          ...history.checkpoints,
          {
...
export function reconstructVersion(history: DocumentHistory, operationIndex: number): GraphicsDocument {
  ...
  const checkpoint =
    [...history.checkpoints].reverse().find(c => c.operationIndex <= target) ?? history.checkpoints[0];
  let document = checkpoint.document;
  for (let i = checkpoint.operationIndex; i < target; i++)
    document = applyOperation(document, history.entries[i].operation);
  return document;
}
