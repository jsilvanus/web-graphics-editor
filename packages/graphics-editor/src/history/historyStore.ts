...
export const createHistoryState = (document: GraphicsDocument): HistoryState => ({
  checkpoints: [{ id: crypto.randomUUID(), timestamp: Date.now(), document, operationIndex: 0 }],
  operations: [],
});
...
export function recordOperation(
  ...
  const checkpoints =
    operations.length % checkpointEvery === 0
      ? [
          ...state.checkpoints,
          {
            id: crypto.randomUUID(),
            timestamp: Date.now(),
            document: state.checkpoints.length
              ? state.checkpoints[state.checkpoints.length - 1].document
              : (undefined as never),
