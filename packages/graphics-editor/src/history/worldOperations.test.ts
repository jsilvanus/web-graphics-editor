...
const world = (): Graphics3DWorld => ({
  ...
  cameras: [{ id: "c", position: [0, 0, 0], rotation: [0, 0, 0], projection: "perspective" }],
  lights: [],
});
const roundTrip = (initial: Graphics3DWorld, op: WorldOperation) =>
  applyWorldOperation(applyWorldOperation(initial, op), op, true);

describe("3D world history operations", () => {
  it("handles mesh CRUD/update", () => {
    const initial = world(),
      mesh = { ...initial.meshes[0], id: "m2" };
    expect(applyWorldOperation(initial, { type: "add-mesh", mesh }).meshes.map(m => m.id)).toEqual([
      "m",
      "m2",
    ]);
    expect(
      applyWorldOperation(initial, { type: "remove-mesh", mesh: initial.meshes[0], index: 0 }).meshes,
    ).toHaveLength(0);
    expect(
      applyWorldOperation(initial, {
        type: "update-mesh",
        meshId: "m",
        from: { name: undefined },
        to: { name: "Cube" },
  ...
  it("handles camera CRUD/update", () => {
    ...
    expect(
      applyWorldOperation(initial, { type: "remove-camera", camera: initial.cameras[0], index: 0 }).cameras,
    ).toHaveLength(0);
    expect(
      applyWorldOperation(initial, { type: "update-camera", cameraId: "c", from: {}, to: { name: "Main" } })
        .cameras[0].name,
    ).toBe("Main");
  });
  it("handles light CRUD/update", () => {
    const initial = world(),
      light = { id: "l", type: "point" as const, intensity: 2 },
      withLight = applyWorldOperation(initial, { type: "add-light", light });
  ...
  it("handles material CRUD/update", () => {
    const initial = world(),
      ...
      withMaterial = applyWorldOperation(initial, { type: "add-material", meshId: "m", material });
    expect(withMaterial.meshes[0].material).toEqual(material);
    expect(
      applyWorldOperation(withMaterial, {
    ...
    expect(
      applyWorldOperation(withMaterial, { type: "remove-material", meshId: "m", material }).meshes[0]
        .material,
    ).toBeUndefined();
  });
  it("stores 3D animation tracks in the world timeline", () => {
    ...
    const updated = applyWorldOperation(withTrack, {
      type: "update-3d-track",
      trackId: "t",
      from: { property: "positionX" },
      to: { property: "positionY" },
    });
  ...
  it("adds, removes, updates and moves 3D keyframes in the world timeline", () => {
    let current = applyWorldOperation(world(), {
      type: "add-3d-track",
      track: { id: "t", targetType: "mesh", targetId: "m", property: "positionX", keyframes: [] },
    });
    ...
    current = applyWorldOperation(current, {
  ...
  it("restores the complete world including timeline on undo", () => {
    ...
    for (const op of [
      { type: "add-3d-track", track },
      { type: "add-3d-keyframe", trackId: "t", keyframe: { id: "k", time: 1, value: 1 } },
    ] as WorldOperation[])
      expect(roundTrip(initial, op)).toEqual(initial);
    expect(invertWorldOperation({ type: "add-3d-track", track })).toEqual({
      type: "remove-3d-track",
      track,
      index: 0,
    });
  });
