import type { GraphicsDocument, Layer, SceneTimeline, Track, Keyframe, LayerClip } from "../types";
...
export interface WorldOperationReference {
...
export type DocumentOperation =
  ...
  | { type: "delete-keyframe"; track: Track; keyframe: Keyframe }
  | { type: "move-keyframe"; trackId: string; keyframeId: string; fromTime: number; toTime: number }
  | {
      type: "set-clip-timing";
      clipId: string;
      from: Pick<LayerClip, "start" | "duration">;
      to: Pick<LayerClip, "start" | "duration">;
    }
  | { type: "batch"; operations: DocumentOperation[] }
  | { type: "world-operation"; worldId: string; operationId: string };
...
function restoreChildren(
  ...
  snapshots: { layer: Layer; index: number }[],
): GraphicsDocument {
  const ids = new Set(snapshots.map(s => s.layer.id)),
    layers = document.layers.filter(layer => !ids.has(layer.id));
  for (const s of [...snapshots].sort((a, b) => a.index - b.index))
    layers.splice(Math.max(0, Math.min(s.index, layers.length)), 0, s.layer);
  return { ...document, layers };
}
function groupedChildren(snapshots: { layer: Layer; index: number }[], groupId: string): Layer[] {
  return snapshots.map(s => ({ ...s.layer, parentId: groupId }));
}
...
function replaceKeyframe(track: Track, keyframe: Keyframe): Track {
  return {
    ...track,
    keyframes: [
...
function updateClip(
  document: GraphicsDocument,
  clipId: string,
  value: Pick<LayerClip, "start" | "duration">,
): GraphicsDocument {
  const timeline = document.timeline;
  if (!timeline) return document;
  return {
    ...
    timeline: {
      ...timeline,
      clips: (timeline.clips ?? []).map(clip => (clip.id === clipId ? { ...clip, ...value } : clip)),
    },
  };
}
export function applyOperation(
  ...
  if (operation.type === "batch")
    return [...(reverse ? [...operation.operations].reverse() : operation.operations)].reduce(
      (c, item) => applyOperation(c, item, reverse),
      document,
    );
  if (operation.type === "world-operation") return document;
  if (operation.type === "set-timeline")
    return { ...document, timeline: reverse ? operation.from : operation.to };
  if (operation.type === "add-keyframe") {
    if (reverse) {
      if (operation.createdTrack)
        return {
          ...document,
          timeline: document.timeline
            ? {
                ...document.timeline,
                tracks: document.timeline.tracks.filter(track => track.id !== operation.track.id),
              }
            : document.timeline,
        };
      return updateTrack(document, operation.track.id, track => ({
        ...track,
        keyframes: track.keyframes.filter(k => k.id !== operation.keyframe.id),
      }));
    }
    return document.timeline
      ? {
          ...
          timeline: {
            ...document.timeline,
            tracks: document.timeline.tracks.some(track => track.id === operation.track.id)
              ? document.timeline.tracks.map(track =>
                  track.id === operation.track.id ? replaceKeyframe(track, operation.keyframe) : track,
                )
  ...
  if (operation.type === "update-keyframe")
    return updateTrack(document, operation.trackId, track => {
      const k = track.keyframes.find(keyframe => keyframe.id === operation.keyframeId);
      return k
        ? replaceKeyframe(track, { ...k, value: reverse ? operation.fromValue : operation.toValue })
        : track;
    });
  if (operation.type === "delete-keyframe")
    return updateTrack(document, operation.track.id, track =>
      reverse
        ? replaceKeyframe(track, operation.keyframe)
        : { ...track, keyframes: track.keyframes.filter(k => k.id !== operation.keyframe.id) },
    );
  if (operation.type === "move-keyframe")
    return updateTrack(document, operation.trackId, track => {
      const k = track.keyframes.find(keyframe => keyframe.id === operation.keyframeId);
      return k
        ? replaceKeyframe(track, { ...k, time: reverse ? operation.fromTime : operation.toTime })
        : track;
    });
  if (operation.type === "set-clip-timing")
    return updateClip(document, operation.clipId, reverse ? operation.from : operation.to);
  ...
  if (operation.type === "remove-layer")
    return reverse
      ? applyOperation(document, { type: "add-layer", layer: operation.layer, index: operation.index })
      : { ...document, layers: document.layers.filter(layer => layer.id !== operation.layer.id) };
  ...
  if (operation.type === "group-layers") {
    ...
    const ids = new Set(operation.children.map(s => s.layer.id)),
      layers = document.layers.filter(layer => !ids.has(layer.id) && layer.id !== operation.group.id);
  ...
  if (operation.type === "ungroup-layer") {
    if (reverse) {
      const without = document.layers.filter(
        layer => !operation.children.some(s => s.layer.id === layer.id) && layer.id !== operation.group.id,
      );
      without.splice(Math.max(0, Math.min(operation.index, without.length)), 0, operation.group);
      return {
        ...document,
  ...
  return {
    ...
    layers: document.layers.map(layer => {
      ...
      if (operation.type === "resize-layer")
        return { ...layer, ...(value as { x: number; y: number; width: number; height: number }) };
...
export function invertOperation(operation: DocumentOperation): DocumentOperation {
  if (operation.type === "batch")
    return { type: "batch", operations: [...operation.operations].reverse().map(invertOperation) };
  if (operation.type === "world-operation") return operation;
  if (operation.type === "add-layer")
    return { type: "remove-layer", layer: operation.layer, index: operation.index ?? 0 };
  if (operation.type === "remove-layer")
    return { type: "add-layer", layer: operation.layer, index: operation.index };
  if (operation.type === "reorder-layer")
  ...
  if (operation.type === "set-timeline")
    return { type: "set-timeline", from: operation.to, to: operation.from };
  if (operation.type === "add-keyframe")
    return { type: "delete-keyframe", track: operation.track, keyframe: operation.keyframe };
  if (operation.type === "delete-keyframe")
    return { type: "add-keyframe", track: operation.track, keyframe: operation.keyframe };
  if (operation.type === "update-keyframe")
    return { ...operation, fromValue: operation.toValue, toValue: operation.fromValue };
  if (operation.type === "move-keyframe")
...
export function diffOperations(before: GraphicsDocument, after: GraphicsDocument): DocumentOperation[] {
  ...
  if (JSON.stringify(before.timeline) !== JSON.stringify(after.timeline))
    ops.push({ type: "set-timeline", from: before.timeline, to: after.timeline });
  const beforeById = new Map(before.layers.map((layer, index) => [layer.id, { layer, index }])),
    afterById = new Map(after.layers.map((layer, index) => [layer.id, { layer, index }]));
  for (const [id, { layer, index }] of beforeById)
    if (!afterById.has(id)) ops.push({ type: "remove-layer", layer, index });
  for (const [id, { layer, index }] of afterById)
    if (!beforeById.has(id)) ops.push({ type: "add-layer", layer, index });
  for (const [id, { layer: beforeLayer, index: beforeIndex }] of beforeById) {
    ...
    if (beforeIndex !== afterIndex)
      ops.push({ type: "reorder-layer", layerId: id, fromIndex: beforeIndex, toIndex: afterIndex });
    if (beforeLayer.x !== afterLayer.x || beforeLayer.y !== afterLayer.y)
      ops.push({
        type: "move-layer",
        layerId: id,
        from: { x: beforeLayer.x, y: beforeLayer.y },
        to: { x: afterLayer.x, y: afterLayer.y },
      });
    if (beforeLayer.width !== afterLayer.width || beforeLayer.height !== afterLayer.height)
      ops.push({
        type: "resize-layer",
        layerId: id,
        from: { x: beforeLayer.x, y: beforeLayer.y, width: beforeLayer.width, height: beforeLayer.height },
        to: { x: afterLayer.x, y: afterLayer.y, width: afterLayer.width, height: afterLayer.height },
      });
    if ((beforeLayer.rotation ?? 0) !== (afterLayer.rotation ?? 0))
      ops.push({
        type: "rotate-layer",
        layerId: id,
        from: beforeLayer.rotation ?? 0,
        to: afterLayer.rotation ?? 0,
      });
    const keys = new Set([...Object.keys(beforeLayer.style ?? {}), ...Object.keys(afterLayer.style ?? {})]);
    for (const key of keys) {
      const from = beforeLayer.style?.[key],
        to = afterLayer.style?.[key];
    ...
    for (const key of Object.keys(afterLayer) as (keyof Layer)[]) {
      ...
      const from = beforeLayer[key],
        to = afterLayer[key];
      if (!Object.is(from, to))
        ops.push({ type: "set-layer-property", layerId: id, property: key as string, from, to });
    }
  }
  return ops;
}
