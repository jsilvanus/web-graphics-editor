...
export function GraphicsEditor({ document: initialDocument, assets = [], onChange }: GraphicsEditorProps) {
  ...
  const [grid, setGrid] = useState(false), [safe, setSafe] = useState(false), [aspectLock, setAspectLock] = useState(true), [assetPicker, setAssetPicker] = useState(false); const [styleClipboard, setStyleClipboard] = useState<Record<string,string|number>|null>(null);
  ...
  const exportJson = useCallback(() => {
    const blob = new Blob([serializeGraphicsDocument({ ...document, timeline })], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = window.document.createElement("a");
    a.href = url; a.download = "graphics.json"; a.click();
  ...
  const interaction = useCanvasInteraction(document, artboardRef, grid, aspectLock, transientChange, selectedIds, activeCompositionId ? compositionTime : timeline.currentTime);
  const drawing = useEditorDrawing(document, commit, select, clear);
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      const target = event.target as HTMLElement | null;
      if (target?.matches("input, textarea, select, [contenteditable=true]")) return;
      if (event.key === "Escape") { drawing.resetTool("select"); setMarquee(null); marqueeRef.current = null; return; }
      if (event.key === "0") { event.preventDefault(); viewport.fit(); return; }
      if (event.key === "1") { event.preventDefault(); viewport.setZoom(1); return; }
      if ((event.ctrlKey || event.metaKey) && (event.key === "+" || event.key === "=")) { event.preventDefault(); viewport.zoomIn(); return; }
      if ((event.ctrlKey || event.metaKey) && event.key === "-") { event.preventDefault(); viewport.zoomOut(); return; }
    };
    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [drawing, viewport]);
  ...
  useTimelinePlayback(playing && context.kind === "main" && !activeCompositionId, setPlaying, transientSeek, timeline);
  useEffect(() => {
    if (!activeCompositionId || !activeComposition) { setCompositionPlaying(false); return; }
  ...
  const enterComposition = useCallback((id: string) => { if (!document.compositions?.some(c => c.id === id)) return; setCompositionPath(path => enterCompositionPath(path, id)); setCompositionTime(0); setCompositionPlaying(false); }, [document.compositions]);
  const exitComposition = useCallback(() => { setCompositionPath(path => exitCompositionPath(path)); setCompositionTime(0); setCompositionPlaying(false); }, []); const copyStyle = useCallback(() => { if (!selectedLayer) return; setStyleClipboard(selectedLayer.style ? { ...selectedLayer.style } : {}); }, [selectedLayer]); const pasteStyle = useCallback(() => { if (!styleClipboard || !selectedIds.size) return; selectedIds.forEach(id => updateLayer(id, { style: { ...styleClipboard } })); }, [styleClipboard, selectedIds, updateLayer]);
  ...
  const onCanvasPointerUp = useCallback(() => {
    ...
    if (drawing.drawingRef.current?.tool === "line" || drawing.drawingRef.current?.tool === "freehand") drawing.finishDrawing(); else { interaction.pointerUp(); transaction.end(document); }
  ...
  const compositorFrame = activeCompositionId ? (() => { const evaluation = evaluateCompositionAtTime(document, activeCompositionId, compositionTime); return evaluation ? { kind: "composition" as const, compositionId: evaluation.composition.id, time: evaluation.time, timeDomain: evaluation.timeDomain, layers: evaluation.layers, renderTree: evaluation.renderTree, videos: evaluation.videos, views3d: evaluation.views3d, evaluation } : composeDocumentAtTime(document, timeline.currentTime); })() : composeDocumentAtTime(document, timeline.currentTime);
  ...
  return <div className="graphics-editor"><style>{KEYFRAMES}</style><GraphicsEditorToolbar grid={grid} safe={safe} canUndo={canUndo} canRedo={canRedo} onUndo={undo} onRedo={redo} onAdd={commands.addLayer} onAdd3DView={add3DView} onDuplicate={commands.duplicateSelected} onDelete={commands.deleteSelected} onToggleGrid={() => setGrid(v => !v)} onToggleSafe={() => setSafe(v => !v)} activeTool={drawing.activeTool} onTool={drawing.resetTool} onGroup={() => { const id = commands.group(selectedIds); if (id) select(id); }} onUngroup={() => { if (primaryId) commands.ungroup(primaryId); }} onForward={() => selectedIds.forEach(commands.bringForward)} onBackward={() => selectedIds.forEach(commands.sendBackward)} onFront={() => selectedIds.forEach(commands.bringToFront)} onBack={() => selectedIds.forEach(commands.sendToBack)} canGroup={selectedIds.size >= 2} canUngroup={!!primaryId && document.layers.some(layer => layer.id === primaryId && layer.type === "group")} onAlign={commands.applyAlign} onDistribute={commands.applyDistribute} onBoolean={commands.applyBoolean} onJoinPaths={commands.applyJoinPaths} onFlattenPath={() => primaryId && commands.flattenPath(primaryId)} canFlatten={!!primaryId && document.layers.some(layer => layer.id === primaryId && layer.type === "path" && !!layer.nodes?.length)} onConvertToPath={() => primaryId && commands.convertToPath(primaryId)} onConvertToShape={() => primaryId && commands.convertToShape(primaryId)} onCopyStyle={copyStyle} onPasteStyle={pasteStyle} canPasteStyle={!!styleClipboard && selectedIds.size > 0} canJoinPaths={selectedIds.size === 2 && [...selectedIds].every(id => { const l=document.layers.find(x=>x.id===id); return l?.type === "path" && !l.closed; })} canAlign={selectedIds.size >= 1} canDistribute={selectedIds.size >= 3} onSaveWegra={saveWegra} onOpenWegra={openWegra} onNewDocument={newDocument} onExportJson={exportJson} onImportJson={importJson}/><div className="ge-layout"><GraphicsEditorCanvas frame={compositorFrame} artboardRef={artboardRef} viewportRef={viewport.hostRef} zoom={viewport.viewport.zoom} panX={viewport.viewport.panX} panY={viewport.viewport.panY} onWheel={viewport.onWheel} onViewportPointerDown={viewport.onPointerDown} onViewportPointerMove={viewport.onPointerMove} onViewportPointerUp={viewport.onPointerUp} onFit={viewport.fit} onZoomIn={viewport.zoomIn} onZoomOut={viewport.zoomOut} onZoomReset={() => viewport.setZoom(1)} layers={activeComposition ? animatedLayers.filter(layer => activeComposition.layerIds.includes(layer.id)) : animatedLayers} selectedIds={selectedIds} grid={grid} safe={safe} background={document.background ?? "#111"} worlds3d={document.worlds3d ?? []} views3d={document.views3d ?? []} onPointerMove={onCanvasPointerMove} onPointerUp={onCanvasPointerUp} onCanvasPointerDown={onCanvasPointerDown} onLayerPointerDown={onLayerPointerDown} onTextCommit={commitText} onTextRunsCommit={commitTextRuns} drawing={drawing.drawing} onDrawingDoubleClick={drawing.finishDrawing} onPathNodes={(id, nodes) => updateLayer(id, { nodes, path: undefined })}/><aside className="ge-properties"><div className="ge-section"><b>Composition</b><button type="button" disabled={!selectedIds.size} onClick={createComposition}>Create composition from selection</button><div style={{display:"flex",gap:4,flexWrap:"wrap"}}><button type="button" onClick={() => { setCompositionPath([]); setCompositionTime(0); setCompositionPlaying(false); }}>Main</button>{compositionPath.map((id,index) => { const c=document.compositions?.find(item=>item.id===id); return <span key={id}>/ <button type="button" onClick={() => { setCompositionPath(compositionPath.slice(0,index+1)); setCompositionTime(0); setCompositionPlaying(false); }}>{c?.name ?? id}</button></span>; })}</div></div><LayerList layers={document.layers} compositions={document.compositions ?? []} selectedIds={selectedIds} onSelect={id => { const layer = document.layers.find(item => item.id === id); if (layer?.locked) return; select(id); }} onMove={commands.moveLayer} onDuplicate={id => { const ids = new Set([id]); const copies = commands.duplicate(ids); if (copies[0]) select(copies[0]); }} onDelete={id => { commands.remove(new Set([id])); if (selectedIds.has(id)) clear(); }} onForward={commands.bringForward} onBackward={commands.sendBackward} onFront={commands.bringToFront} onBack={commands.sendToBack} onGroup={() => { const id = commands.group(selectedIds); if (id) select(id); }} onUngroup={commands.ungroup} onToggleVisibility={id => setDocument(d => ({ ...d, layers: d.layers.map(layer => layer.id === id ? { ...layer, visible: layer.visible === false ? true : false } : layer) }), true)} onToggleLock={id => setDocument(d => ({ ...d, layers: d.layers.map(layer => layer.id === id ? { ...layer, locked: !layer.locked } : layer) }), true)} onRename={(id, name) => { setDocument(d => ({ ...d, layers: d.layers.map(layer => layer.id === id ? { ...layer, name: name.trim() || undefined } : layer) }), true); }}/>{selectedLayer?.type === "composition" ? <CompositionProperties layer={selectedLayer} composition={document.compositions?.find(c => c.id === selectedLayer.compositionId)} compositions={document.compositions ?? []} onLayer={patch => changeLayer(selectedLayer.id, patch)} onComposition={patch => { const id=selectedLayer.compositionId; if (!id) return; setDocument(d => ({...d, compositions:(d.compositions ?? []).map(c => c.id===id ? {...c,...patch} : c)}), true); }} onEnter={() => selectedLayer.compositionId && enterComposition(selectedLayer.compositionId)} /> : selected3DView && selected3DWorld ? <div className="ge-section"><strong>{selected3DView.name ?? "3D View"}</strong><label style={{ display: "block", marginTop: 8 }}>World <select value={selected3DView.worldId} onChange={event => { const selectedWorld = document.worlds3d?.find(item => item.id === event.target.value); update3DView(selected3DView.id, { worldId: event.target.value, cameraId: selectedWorld?.cameras[0]?.id ?? selected3DView.cameraId }); }}>{(document.worlds3d ?? []).map(item => <option key={item.id} value={item.id}>{item.name ?? item.id}</option>)}</select></label><label style={{ display: "block", marginTop: 8 }}>Camera <select value={selected3DView.cameraId} onChange={event => update3DView(selected3DView.id, { cameraId: event.target.value })}>{selected3DWorld.cameras.map((camera: Graphics3DCamera) => <option key={camera.id} value={camera.id}>{camera.name ?? camera.id}</option>)}</select></label><label style={{ display: "block", marginTop: 8 }}>Render <select value={selected3DView.renderMode ?? "auto"} onChange={event => update3DView(selected3DView.id, { renderMode: event.target.value as Graphics3DView["renderMode"] })}><option value="auto">Auto / prerender in 2D</option><option value="prerender">Prerender</option><option value="live">Live</option></select></label><Graphics3DRenderSettingsPanel settings={selected3DView.renderSettings} onChange={renderSettings => update3DView(selected3DView.id, { renderSettings })}/><button type="button" style={{ marginTop: 10, width: "100%" }} onClick={editSelectedWorld}>Edit World Animation</button><div style={{ marginTop: 10, fontSize: 12, opacity: .75 }}>World objects: {selected3DWorld.meshes.length}</div>{selectedCamera && <div style={{ marginTop: 6, fontSize: 12, opacity: .65 }}>Camera FOV: {selectedCamera.fov}°</div>}</div> : selectedLayer ? <LayerProperties layer={selectedLayer} assets={projectAssets} aspectLock={aspectLock} assetPicker={assetPicker} onLayer={changeLayer} onStyle={changeStyle} onChooseAsset={asset => { updateLayer(selectedLayer.id, { src: asset.url }); setAssetPicker(false); }} onToggleAssetPicker={() => setAssetPicker(v => !v)} onAspectLock={setAspectLock} onFont={addFont} onAsset={asset => setDocument(d => ({...d,assets:[...(d.assets ?? []).filter(a => a.id !== asset.id),asset]}))} onOffset={(id,distance) => commands.applyOffset(id,distance)}/> : <div className="ge-section"><span>Select a layer.</span></div>}</aside></div>{context.kind === "world" && world ? <WorldTimelinePanel world={world} timeline={worldTimeline} currentTime={worldCurrentTime} onSeek={seek} onChange={updateWorldTimeline} onExit={exitWorld}/> : activeComposition ? <CompositionTimelinePanel composition={activeComposition} layers={document.layers.filter(layer => activeComposition.layerIds.includes(layer.id))} compositions={document.compositions ?? []} assets={document.assets ?? []} onLayerChange={layer => setDocument({...document,layers:document.layers.map(item=>item.id===layer.id?layer:item)},true)} currentTime={compositionTime} onSeek={time => setCompositionTime(Math.max(0, Math.min(activeComposition.duration ?? 10, time)))} onChange={composition => setDocument({...document,compositions:(document.compositions ?? []).map(c=>c.id===composition.id?composition:c)}, true)}/> : <SceneTimelinePanel timeline={timeline} layers={document.layers} worlds3d={document.worlds3d ?? []} views3d={document.views3d ?? []} onChange={changeTimeline} onSeek={seek}/>}<div className="ge-playback"><button onClick={() => activeCompositionId ? setCompositionTime(0) : seek(0)}>⏮</button><button onClick={() => activeCompositionId ? setCompositionPlaying(v => !v) : setPlaying(v => !v)} disabled={context.kind !== "main"}>{activeCompositionId ? (compositionPlaying ? "⏸" : "▶") : (playing ? "⏸" : "▶")}</button><span>{context.kind === "world" ? `${worldCurrentTime.toFixed(2)}s / ${(worldTimeline.duration ?? 0).toFixed(2)}s` : activeCompositionId ? `${compositionTime.toFixed(2)}s / ${(activeComposition?.duration ?? 10).toFixed(2)}s` : `${timeline.currentTime.toFixed(2)}s / ${total.toFixed(2)}s`}</span></div></div>;
}
