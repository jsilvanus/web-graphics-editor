...
export type TextOverflow = "visible" | "hidden";
...
export interface Layer {
  ...
  playbackRate?: number;
  loop?: boolean;
  viewportOverrides?: Record<string, ViewportOverride>;
...
export type Easing =
  "linear" | "ease-in" | "ease-out" | "ease-in-out" | "step-start" | "step-end" | "cubic-bezier";
...
export interface EasingParameters {
  mode: Easing;
  bezier?: [number, number, number, number];
}
export interface InterpolationOptions {
  mode?: InterpolationMode;
  easing?: EasingParameters;
  colorSpace?: ColorInterpolationSpace;
}
export interface AnimationKeyframe<T extends AnimationValue = AnimationValue> {
  id: string;
  time: number;
  value: T;
...
export type Graphics3DAnimatedProperty =
  | "positionX"
  | "positionY"
  | "positionZ"
  | "rotationX"
...
export interface Graphics3DWorldTimeline {
  duration?: number;
...
export type SceneTransitionType =
  "cut" | "fade" | "dissolve" | "slide-left" | "slide-right" | "slide-up" | "slide-down";
...
export interface WorldTimeMapping {
  offset: number;
  rate: number;
  loop?: boolean;
  inPoint?: number;
...
export interface Graphics3DTransform {
  position: [number, number, number];
  rotation: [number, number, number];
  scale: [number, number, number];
}
export interface Graphics3DMaterial {
  color?: string;
  opacity?: number;
  metalness?: number;
  roughness?: number;
...
export interface Graphics3DMesh {
  id: string;
  name?: string;
  geometry: Graphics3DMeshGeometry;
  transform: Graphics3DTransform;
  opacity?: number;
  visible?: boolean;
  material?: Graphics3DMaterial;
  provenance?: Provenance;
}
export interface Graphics3DLight {
  id: string;
...
export interface Graphics3DCamera {
  id: string;
  name?: string;
  position: [number, number, number];
  rotation: [number, number, number];
...
export interface Graphics3DVisibility {
  mode: "all" | "include" | "exclude";
  objects: string[];
}
export type Graphics3DRenderMode = "auto" | "prerender" | "live";
export type Graphics3DResolutionMode = "auto" | "custom";
export interface Graphics3DRenderSettings {
  resolutionMode?: Graphics3DResolutionMode;
...
export interface Graphics3DWorld {
  id: string;
  name?: string;
...
export interface Graphics3DView {
  id: string;
  name?: string;
  worldId: string;
  cameraId: string;
...
export interface GraphicsOutput {
  ...
  name: string;
  viewportId?: string;
  playback: OutputPlaybackMode;
  background: OutputBackgroundMode;
  inTransition?: OutputTransition;
  outTransition?: OutputTransition;
  loop?: boolean;
  autoplay?: boolean;
  editable?: boolean;
  liveControl?: boolean;
