...
function mesh(
  ...
  vertices: number[],
  indices: number[],
  transform: Graphics3DTransform = identity,
): Graphics3DMesh {
  return {
    id,
...
export function createCylinderMesh(
  ...
  for (let y = 0; y <= 1; y++)
    for (let i = 0; i < n; i++) {
      ...
      vertices.push(Math.cos(a) * radius, (y - 0.5) * height, Math.sin(a) * radius);
    }
  ...
  for (let i = 0; i < n; i++) {
    ...
    indices.push(bottom, bn, b);
    indices.push(top, t, tn);
  }
...
export function createSphereMesh(
  ...
  const s = Math.max(3, Math.floor(segments)),
    r = Math.max(2, Math.floor(rings)),
    vertices: number[] = [],
    indices: number[] = [];
  for (let y = 0; y <= r; y++) {
    const phi = (y / r) * Math.PI;
    for (let x = 0; x < s; x++) {
...
export function createConeMesh(
  ...
  for (let i = 0; i < n; i++) {
    const a = (i / n) * Math.PI * 2;
    vertices.push(Math.cos(a) * radiusTop, height / 2, Math.sin(a) * radiusTop);
  }
  const bottom = vertices.length / 3;
  vertices.push(0, -height / 2, 0);
  const top = vertices.length / 3;
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (radiusTop > 0) indices.push(top, n + i, n + j);
  }
  return mesh(id, vertices, indices, transform);
}
...
export function createBoxMesh(
  ...
  const indices = [
    0, 2, 1, 0, 3, 2, 4, 5, 6, 4, 6, 7, 0, 1, 5, 0, 5, 4, 2, 3, 7, 2, 7, 6, 0, 4, 7, 0, 7, 3, 1, 2, 6, 1, 6,
    5,
  ];
  return mesh(id, vertices, indices, transform);
}
