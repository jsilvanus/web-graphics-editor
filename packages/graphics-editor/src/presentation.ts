...
export function resolveComposition(
  ...
  const composition = findComposition(document, compositionId);
  if (!composition) return undefined;
  const byId = new Map(document.layers.map(layer => [layer.id, layer]));
  const layers = composition.layerIds
    .map(id => byId.get(id))
    .filter((layer): layer is Layer => Boolean(layer))
    .map(layer => ({ ...layer, sourceLayerId: layer.id }));
...
export function resolveViewportComposition(
  document: GraphicsDocument,
  viewportId: string | undefined,
  compositionId?: string,
): ResolvedComposition | undefined {
  const viewport = findViewport(document, viewportId);
  const selectedId = compositionId ?? viewport?.compositionIds?.[0];
  const resolved = resolveComposition(document, selectedId);
...
/** Return the scene containing the requested presentation time. Scene ends are exclusive. */
export function sceneAtTime(timeline: SceneTimeline | undefined, time: number): Scene | undefined {
  ...
  return timeline.scenes.find(scene => t >= scene.start && t < scene.start + scene.duration);
}
export function resolveScene(
  ...
  const scene = sceneAtTime(timeline, time);
  if (!scene) return undefined;
  const duration = timeline!.scenes.reduce((max, s) => Math.max(max, s.start + s.duration), 0);
  const resolvedTime = timeline!.loop && duration > 0 ? Math.max(0, time) % duration : Math.max(0, time);
  const resolved = resolveViewportComposition(document, viewportId, scene.compositionId);
  if (!resolved) return undefined;
  return {
    scene,
...
export function resolveOutput(
  ...
  outputId: string,
  time: number = 0,
): ResolvedScene | ResolvedComposition | undefined {
  const output = document.outputs?.find(item => item.id === outputId);
