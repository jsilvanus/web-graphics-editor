...
export function layerStyle(layer:Layer,selected:boolean):CSSProperties{const css:CSSProperties={};for(const[key,value]of Object.entries(layer.style??{}))(css as Record<string,unknown>)[key.replace(/-([a-z])/g,(_,c)=>c.toUpperCase())]=value;const gradient=gradientToCss(layer.gradient);if(gradient!=="none")css.backgroundImage=gradient;return{position:"absolute",left:layer.x,top:layer.y,width:layer.width,height:layer.height,boxSizing:"border-box",userSelect:"none",cursor:"move",outline:selected?"3px solid #38bdf8":undefined,opacity:Math.max(0,Math.min(1,layer.opacity??1)),animation:layer.animation||undefined,transform:[layer.rotation?`rotate(${layer.rotation}deg)`:"",layer.skewX?`skewX(${layer.skewX}deg)`:"",layer.skewY?`skewY(${layer.skewY}deg)`:""].filter(Boolean).join(" ")||undefined,transformOrigin:layer.transformOrigin?`${layer.transformOrigin.x}px ${layer.transformOrigin.y}px`:undefined,...css}}
...
export interface FlattenedPath { points: Point[]; closed: boolean; }
...
function flattenCubic(p0:Point,p1:Point,p2:Point,p3:Point,tolerance:number,out:Point[],depth=0):void {
  const flatness=Math.max(pointLineDistance(p1,p0,p3),pointLineDistance(p2,p0,p3));
  if(flatness<=tolerance||depth>=12){out.push(p3);return;}
  const p01={x:(p0.x+p1.x)/2,y:(p0.y+p1.y)/2},p12={x:(p1.x+p2.x)/2,y:(p1.y+p2.y)/2},p23={x:(p2.x+p3.x)/2,y:(p2.y+p3.y)/2};
  const p012={x:(p01.x+p12.x)/2,y:(p01.y+p12.y)/2},p123={x:(p12.x+p23.x)/2,y:(p12.y+p23.y)/2},mid={x:(p012.x+p123.x)/2,y:(p012.y+p123.y)/2};
...
function flattenQuadratic(p0:Point,p1:Point,p2:Point,tolerance:number,out:Point[],depth=0):void {
  ...
  if(flatness<=tolerance||depth>=12){out.push(p2);return;}
  const p01={x:(p0.x+p1.x)/2,y:(p0.y+p1.y)/2},p12={x:(p1.x+p2.x)/2,y:(p1.y+p2.y)/2},mid={x:(p01.x+p12.x)/2,y:(p01.y+p12.y)/2};
...
export function flattenPathNodes(nodes:PathNode[],closed=false,tolerance=0.75):FlattenedPath {
  ...
  const points:Point[]=[{x:nodes[0].x,y:nodes[0].y}], count=closed?nodes.length:nodes.length-1;
...
function lineIntersection(a:Point,b:Point,c:Point,d:Point):Point|null {
  const abx=b.x-a.x,aby=b.y-a.y,cdx=d.x-c.x,cdy=d.y-c.y,denominator=abx*cdy-aby*cdx;
...
function offsetPolyline(points:Point[],distance:number,closed:boolean):Point[] {
  ...
  const n=points.length,result:Point[]=[];
  const normal=(a:Point,b:Point)=>{const dx=b.x-a.x,dy=b.y-a.y,len=Math.hypot(dx,dy)||1;return{x:-dy/len*distance,y:dx/len*distance};};
  for(let i=0;i<n;i++){
    if(!closed&&i===0){const q=normal(points[0],points[1]);result.push({x:points[0].x+q.x,y:points[0].y+q.y});continue;}
    if(!closed&&i===n-1){const q=normal(points[n-2],points[n-1]);result.push({x:points[n-1].x+q.x,y:points[n-1].y+q.y});continue;}
    const prev=points[(i-1+n)%n],cur=points[i],next=points[(i+1)%n],n1=normal(prev,cur),n2=normal(cur,next);
    const hit=lineIntersection({x:prev.x+n1.x,y:prev.y+n1.y},{x:cur.x+n1.x,y:cur.y+n1.y},{x:cur.x+n2.x,y:cur.y+n2.y},{x:next.x+n2.x,y:next.y+n2.y});
    result.push(hit&&Number.isFinite(hit.x)&&Number.isFinite(hit.y)?hit:{x:cur.x+(n1.x+n2.x)/2,y:cur.y+(n1.y+n2.y)/2});
  }
  return result;
}

/** Offset straight and Bézier paths by adaptively flattening curves first. */
export function offsetPathNodes(nodes:PathNode[],distance:number,closed=false,tolerance=0.75):PathNode[] {
  ...
  return offsetPolyline(flattened.points,distance,closed).map(p=>({x:p.x,y:p.y,kind:"corner" as const}));
}
