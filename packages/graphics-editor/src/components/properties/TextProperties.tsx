...
export const TextProperties: FC<{ layer: Layer; onStyle: (key: string, value: string) => void; onText: (text: string) => void }> = ({ layer, onStyle, onText }) => (
  <div className="ge-section">
    <b>Text</b>
    <label>Content<textarea value={layer.text ?? ""} onChange={e => onText(e.target.value)} /></label>
    ...
    <div className="ge-two"><label>Weight<select value={styleValue(layer, "font-weight", "700")} onChange={e => onStyle("font-weight", e.target.value)}><option>normal</option><option>bold</option><option>400</option><option>500</option><option>600</option><option>700</option><option>800</option><option>900</option></select></label><label>Align<select value={styleValue(layer, "text-align", "center")} onChange={e => onStyle("text-align", e.target.value)}><option>left</option><option>center</option><option>right</option></select></label></div>
