...
export const TextProperties: FC<{ layer: Layer; assets: GraphicsAsset[]; onStyle: (key: string, value: string) => void; onText: (text: string) => void; onFont: (asset: GraphicsAsset) => void }> = ({ layer, assets, onStyle, onText, onFont }) => {
  ...
  return <div className="ge-section">
    ...
    <div className="ge-text-toolbar" role="toolbar" aria-label="Typography" style={{ display: "flex", flexWrap: "wrap", gap: 4, margin: "6px 0" }}>
      <button type="button" aria-pressed={weight === "700" || weight === "800" || weight === "900"} title="Bold" onClick={() => setTextStyle("weight", weight === "700" || weight === "800" || weight === "900" ? "400" : "700")}>B</button>
      <button type="button" aria-pressed={fontStyle === "italic"} title="Italic" onClick={() => setTextStyle("style", fontStyle === "italic" ? "normal" : "italic")}>I</button>
      <button type="button" aria-pressed={align === "left"} title="Align left" onClick={() => setTextStyle("align", "left")}>L</button>
      <button type="button" aria-pressed={align === "center"} title="Align center" onClick={() => setTextStyle("align", "center")}>C</button>
      <button type="button" aria-pressed={align === "right"} title="Align right" onClick={() => setTextStyle("align", "right")}>R</button>
      <span style={{ width: 1, background: "currentColor", opacity: .25 }} />
      <label style={{ margin: 0 }}>Size <input aria-label="Font size" type="number" min="1" value={ts.fontSize ?? parseFloat(styleValue(layer, "font-size", "72px"))} onChange={e => setTextStyle("size", e.target.value)} style={{ width: 64 }} /></label>
    </div>

    <label>Font family<select value={ts.fontAssetId ?? ""} onChange={e => { const a = fonts.find(x => x.id === e.target.value); if (a) { onFont(a); setTextStyle("family", String(a.metadata?.family ?? a.name)); } else setTextStyle("family", e.target.value); }}><option value="">{ts.fontFamily ?? styleValue(layer, "font-family", "Arial, sans-serif")}</option><option value="Arial, sans-serif">Arial</option><option value="Helvetica, Arial, sans-serif">Helvetica</option><option value="Georgia, serif">Georgia</option><option value="Times New Roman, serif">Times New Roman</option><option value="Verdana, sans-serif">Verdana</option>{fonts.map(a => <option key={a.id} value={a.id}>{String(a.metadata?.family ?? a.name)}</option>)}</select></label>
    ...
    <div className="ge-two"><label>Weight<select value={weight} onChange={e => setTextStyle("weight", e.target.value)}>{["300", "400", "500", "600", "700", "800", "900"].map(x => <option key={x}>{x}</option>)}</select></label><label>Style<select value={fontStyle} onChange={e => setTextStyle("style", e.target.value)}><option value="normal">Normal</option><option value="italic">Italic</option></select></label></div>
    <div className="ge-two"><label>Align<select value={align} onChange={e => setTextStyle("align", e.target.value)}><option value="left">Left</option><option value="center">Center</option><option value="right">Right</option></select></label><label>Vertical<select value={ts.verticalAlign ?? "top"} onChange={e => setTextStyle("vertical-align", e.target.value as VerticalAlign)}><option value="top">Top</option><option value="middle">Middle</option><option value="bottom">Bottom</option></select></label></div>
