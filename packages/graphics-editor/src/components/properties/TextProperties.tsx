...
export const TextProperties: FC<{ layer: Layer; assets: GraphicsAsset[]; onStyle: (key: string, value: string) => void; onText: (text: string) => void; onFont: (asset: GraphicsAsset) => void }> = ({ layer, assets, onStyle, onText, onFont }) => {
  ...
  return <div className="ge-section">
    ...
    <div className="ge-text-toolbar" role="toolbar" aria-label="Typography" style={{ display: "flex", flexWrap: "wrap", gap: 4, margin: "6px 0" }}>
      ...
      <button type="button" aria-pressed={fontStyle === "italic"} title="Italic" onClick={() => setTextStyle("font-style", fontStyle === "italic" ? "normal" : "italic")}>I</button><button type="button" aria-pressed={styleValue(layer, "text-decoration", "none") === "underline"} title="Underline" onClick={() => setTextStyle("text-decoration", styleValue(layer, "text-decoration", "none") === "underline" ? "none" : "underline")}>U</button><button type="button" aria-pressed={styleValue(layer, "text-decoration", "none") === "line-through"} title="Strikethrough" onClick={() => setTextStyle("text-decoration", styleValue(layer, "text-decoration", "none") === "line-through" ? "none" : "line-through")}>S</button>
