...
export const GradientProperties: FC<{
  gradient?: Gradient;
  ...
  const setG = (patch: Partial<Gradient>) =>
    onGradient({
      ...
      ...gradient,
      ...patch,
      stops: gradient?.stops ?? defaultGradient().stops,
    });
  const setStop = (i: number, patch: Partial<GradientStop>) =>
    onGradient({ ...gradient!, stops: gradient!.stops.map((s, n) => (n === i ? { ...s, ...patch } : s)) });
  return (
        ...
        <select
          ...
          onChange={e =>
            e.target.value === "none"
              ...
              : setG({ type: e.target.value as Gradient["type"] })
          }
        >
            ...
            <div className="ge-two">
              <label>
                ...
                <input
                  ...
                  min="0"
                  max="100"
                  onChange={e => setG({ cx: Number(e.target.value) })}
                />
              </label>
              <label>
                Center Y
                <input
                  ...
                  min="0"
                  max="100"
                  onChange={e => setG({ cy: Number(e.target.value) })}
                />
              </label>
            </div>
          )}
          <div>
            ...
            {gradient.stops.map((s, i) => (
              <div className="ge-two" key={i}>
                ...
                <input
                  ...
                  onChange={e => setStop(i, { opacity: Number(e.target.value) / 100 })}
                />
                {gradient.stops.length > 2 && (
                  <button
                    type="button"
                    onClick={() =>
                      onGradient({ ...gradient, stops: gradient.stops.filter((_, n) => n !== i) })
                    }
                  >
                    ×
                  </button>
            ...
            <button
              ...
              onClick={() =>
                onGradient({
                  ...gradient,
                  stops: [...gradient.stops, { offset: 0.5, color: "#808080", opacity: 1 }],
                })
              }
            >
              + Stop
