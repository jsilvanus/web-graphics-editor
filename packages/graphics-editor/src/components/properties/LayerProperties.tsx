...
export const LayerProperties: FC<{
  ...
  onOffset?: (id: string, distance: number) => void;
  ...
  return (
    <>
      ...
      <div className="ge-section">
        <label>
          ...
          <input
            ...
            onChange={e => style("opacity", e.target.value)}
          />
        </label>
      </div>
      {layer.type === "text" && (
      ...
      {(layer.type === "rectangle" || layer.type === "ellipse") && (
        <ShapeProperties layer={layer} onStyle={style} onGradient={gradient => patch({ gradient })} />
      )}{" "}
      {(layer.type === "line" || layer.type === "path") && (
        <PathProperties
          layer={layer}
          onStyle={style}
          onLayer={patch}
          onGradient={gradient => patch({ gradient })}
          onOffset={distance => onOffset?.(layer.id, distance)}
        />
      )}{" "}
      {layer.type === "video" && (
        <VideoProperties layer={layer} assets={assets} onLayer={patch} onAsset={onAsset} />
      )}{" "}
      {layer.type === "image" && (
        <ImageProperties
          layer={layer}
          assets={assets}
          onLayer={patch}
          onTogglePicker={onToggleAssetPicker}
