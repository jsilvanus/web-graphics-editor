...
export function Graphics3DViewInspector({
  view,
  worlds,
  onChange,
  onEditWorld,
}: Graphics3DViewInspectorProps) {
  ...
  return (
    <aside style={{ padding: 12, display: "grid", gap: 8, fontSize: 13 }}>
        ...
        <select
          ...
          onChange={e =>
            onChange({
              ...
              cameraId: worlds.find(w => w.id === e.target.value)?.cameras[0]?.id ?? view.cameraId,
            })
          }
        >
          {worlds.map(item => (
      ...
      <label>
        Camera{" "}
        <select value={view.cameraId} onChange={e => onChange({ cameraId: e.target.value })}>
          {cameras.map(camera => (
      ...
      <fieldset style={{ border: "1px solid #ccc", padding: 8 }}>
        ...
        <label style={{ display: "block" }}>
          World time at view start{" "}
          <input
            type="number"
            step="0.1"
            value={mapping.offset}
            onChange={e => setMapping({ offset: Number(e.target.value) || 0 })}
          />
        </label>
        <label style={{ display: "block" }}>
          Speed{" "}
          <input
            ...
            step="0.1"
            value={mapping.rate}
            onChange={e => setMapping({ rate: Number(e.target.value) || 0 })}
          />{" "}
          ×
        </label>
        <label style={{ display: "block" }}>
        ...
        <label style={{ display: "block" }}>
          ...
          <input
            ...
            min="0"
            step="0.1"
            value={mapping.inPoint ?? 0}
            onChange={e => setMapping({ inPoint: Number(e.target.value) || 0 })}
          />
        </label>
        <label style={{ display: "block" }}>
          Out point{" "}
          <input
            ...
            min="0"
            step="0.1"
            value={mapping.outPoint ?? ""}
            onChange={e =>
              setMapping({ outPoint: e.target.value === "" ? undefined : Number(e.target.value) })
            }
          />
        </label>
      </fieldset>
      <p style={{ margin: 0, opacity: 0.7 }}>
        The view is a window into the selected World. World animation is edited in the World; this view
        controls how it is sampled.
      </p>
    </aside>
  );
}
