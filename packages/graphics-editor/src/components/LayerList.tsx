...
export const LayerList: FC<{
  ...
  const renderLayer = (layer: Layer, depth: number): ReactNode => {
    const children = getChildLayers(layers, layer.id), isGroup = layer.type === "group", isCollapsed = collapsed.has(layer.id);
  ...
  return <div className="ge-section">
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}><b>Layers</b>{onGroup && selectedIds.size >= 2 && <button type="button" onClick={onGroup}>Group</button>}</div>
    <div className="ge-layer-list">{roots.map(layer => renderLayer(layer, 0))}{!layers.length && <span>No layers.</span>}</div>
  </div>;
};
