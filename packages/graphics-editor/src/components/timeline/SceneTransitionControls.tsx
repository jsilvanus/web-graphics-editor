        ...
        i < scenes.length - 1 && (
          <div className="ge-scene-control" key={scene.id}>
            ...
            <select
              ...
              onChange={e =>
                onChange(scene.id, e.target.value as SceneTransitionType, scene.transition?.duration ?? 0.5)
              }
            >
              {TRANSITIONS.map(t => (
            ...
            <input
              ...
              max={scene.duration}
              step=".1"
              value={scene.transition?.duration ?? 0}
              onChange={e => onChange(scene.id, scene.transition?.type ?? "cut", Number(e.target.value) || 0)}
            />
            <span>s</span>
          </div>
        ),
