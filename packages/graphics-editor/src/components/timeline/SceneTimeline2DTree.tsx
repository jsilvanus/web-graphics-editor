...
export const SceneTimeline2DTree: FC<SceneTimeline2DTreeProps> = ({
  scenes,
  layers,
  expanded,
  onToggle,
  ...
  <>
    {scenes.map((scene, index) => (
      <div className="ge-scene-tree" key={scene.id}>
        <button
          className="ge-tree-row ge-tree-scene"
          onClick={() => {
            onSelectScene(scene.id);
            onToggle(`scene:${scene.id}`);
          }}
        >
          {expanded.has(`scene:${scene.id}`) ? "▾" : "▸"} {scene.name}
        </button>
        {sceneAction?.(scene, index)}
        {expanded.has(`scene:${scene.id}`) &&
          layers
            .filter(layer => layer.type !== "group" && layer.type !== "3d-view")
            .map(layer => renderLayer(layer, scene))}
      </div>
    ))}
  </>
);

export const DEFAULT_2D_PROPERTIES: Timeline2DProperty[] = [
  "x",
  "y",
  "width",
  "height",
