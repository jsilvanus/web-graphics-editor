...
export function ThreeDWorkspaceInspector({ world, selectedId, cameraId, onSelect, onUpdateMesh, onUpdateCamera, onAddBox, onDeleteMesh, onCameraChange, onAddCamera, onAddLight }: ThreeDWorkspaceInspectorProps) {
  ...
  return <aside style={{ padding: 12, overflow: "auto", borderLeft: "1px solid #30343b", fontSize: 13 }}>
    ...
    <h4>Objects</h4>
    {world.meshes.map(mesh => <button key={mesh.id} onClick={() => onSelect(mesh.id)} style={{ display: "block", width: "100%", textAlign: "left", marginBottom: 4, fontWeight: mesh.id === selectedId ? 700 : 400 }}>{mesh.name ?? mesh.id}</button>)}
