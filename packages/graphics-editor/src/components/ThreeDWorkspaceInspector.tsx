...
export interface ThreeDWorkspaceInspectorProps { world: Graphics3DWorld; selectedId: string | null; cameraId: string; editCamera: boolean; onEditCamera: (editing: boolean) => void; onSelect: (id: string | null) => void; onUpdateMesh: (id: string, patch: Partial<Omit<Graphics3DMesh, "id">>) => void; onUpdateCamera: (id: string, patch: Partial<Omit<Graphics3DCamera, "id">>) => void; onAddBox: () => void; onDeleteMesh: () => void; onCameraChange: (id: string) => void; onAddCamera: () => void; onAddLight: () => void; }
type Axis = "x" | "y" | "z"; const AXES: Axis[] = ["x", "y", "z"];
function VectorFields({label,values,onChange,step=0.1}:{label:string;values:[number,number,number];onChange:(index:number,value:number)=>void;step?:number}) { return <fieldset style={{border:0,padding:0,margin:"8px 0"}}><legend style={{fontWeight:600}}>{label}</legend>{AXES.map((axis,i)=><label key={axis} style={{display:"block"}}>{axis.toUpperCase()} <input type="number" step={step} value={values[i]} onChange={e=>onChange(i,Number(e.target.value))}/></label>)}</fieldset>; }
