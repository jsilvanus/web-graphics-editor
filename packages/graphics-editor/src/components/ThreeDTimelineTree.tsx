...
const MESH_PROPERTIES: Graphics3DAnimatedProperty[] = [
  "positionX",
  "positionY",
  "positionZ",
  "rotationX",
...
export const ThreeDTimelineTree: FC<ThreeDTimelineTreeProps> = ({
  views,
  viewData,
  worlds,
  expanded,
        ...
        <div className="ge-3d-tree" key={layer.id}>
          ...
          {expanded.has(key) && (
            <div className="ge-3d-children">
              ...
              {(["opacity", "visibility"] as Graphics3DAnimatedProperty[]).map(p =>
                renderTrack("view", viewId, p),
              )}
              ...
              {world?.cameras.length ? (
                world.cameras.map(camera => (
