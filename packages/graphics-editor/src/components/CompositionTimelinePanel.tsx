...
export const CompositionTimelinePanel: FC<{
  ...
  const add = (layer: Layer, property: string) => {
    ...
    const value =
      Number(
        (layer as unknown as Record<string, unknown>)[property] ??
          (property === "opacity" ? layer.opacity : 0),
      ) || 0;
    update([
      ...tracks,
      {
        ...
        targetId: layer.id,
        property,
        keyframes: [createKeyframe(currentTime, value)],
      },
    ]);
  };
  ...
  const scrub = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const move = (moveEvent: PointerEvent) => seekFromClientX(moveEvent.clientX, event.currentTarget);
    const up = () => {
  ...
  const dragVideo = (layer: Layer, kind: "move" | "in" | "out", startX: number, bar: HTMLElement) => {
    const initial = {
      offset: Math.max(0, layer.timeOffset ?? 0),
      sourceIn: Math.max(0, layer.sourceIn ?? 0),
    ...
    const apply = (x: number) => {
      ...
      if (kind === "move") {
        const nextOffset = Math.max(
          0,
          Math.min(Math.max(0, total - sourceSpan / rate), initial.offset + delta),
        );
        ...
        updateVideo(layer, {
          sourceOut: Math.max(
            minOut,
            Math.min(duration, initial.sourceOut === undefined ? duration : initial.sourceOut + delta),
          ),
        });
      }
    };
  ...
  return (
    <section className="ge-timeline" aria-label="Composition timeline">
      <div className="ge-timeline-subhead">
        {composition.name} timeline · {currentTime.toFixed(2)}s / {total.toFixed(2)}s
      </div>
      <div
        className="ge-timeline-ruler"
        style={{
          position: "relative",
          height: 22,
          display: "flex",
      ...
      >
        ...
        <span
          ...
          style={{
            position: "absolute",
            left: `${Math.max(0, Math.min(100, (currentTime / total) * 100))}%`,
            top: 0,
            bottom: 0,
            width: 2,
      ...
      <div className="ge-timeline-body" style={{ position: "relative" }} onPointerDown={scrub}>
        <div
          aria-hidden="true"
          style={{
            position: "absolute",
        ...
        <div className="ge-track-list">
          {layers
            ...
            .map(layer => (
              <div key={layer.id} className="ge-tree-object">
                <b>{layer.name ?? layer.text ?? layer.id}</b>
                {["x", "y", "width", "height", "rotation", "opacity"].map(property => {
                  const t = tracks.find(x => x.targetId === layer.id && x.property === property);
                  return (
                    <TimelineTrackRow
                      key={property}
                      label={property}
                      keyframeCount={t?.keyframes.length ?? 0}
                      onAdd={() => add(layer, property)}
                    >
                      {t && (
                        <div className="ge-track-key-area">
                          <TimelineKeyMarkers
                            ...
                            onSelect={keyId => setSelected({ trackId: t.id, keyId })}
                            onMove={(keyId, time) =>
                              update(tracks.map(x => (x.id === t.id ? moveKeyframe(x, keyId, time) : x)))
                            }
                            onDelete={keyId =>
                              update(tracks.map(x => (x.id === t.id ? removeKeyframe(x, keyId) : x)))
                            }
                          />
                        </div>
                      )}
      ...
      <div style={{ marginTop: 12 }}>
        <b>Video clips</b>
        {layers
          .filter(l => l.type === "video" && l.videoAssetId)
          .map(layer => {
            ...
            return (
              <div key={layer.id} style={{ marginTop: 8 }} onPointerDown={e => e.stopPropagation()}>
                <div
                  style={{
                    display: "flex",
                    justifyContent: "space-between",
                ...
                >
                  <b>{layer.name ?? layer.videoAssetId}</b>
                  <span>
                    {start.toFixed(2)}s → {(start + width).toFixed(2)}s
                  </span>
                </div>
                <div
                  style={{
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      width: widthPct + "%",
                      height: "100%",
                      background: selectedVideo ? "rgba(100,160,255,.75)" : "rgba(100,160,255,.55)",
                      borderRadius: 3,
                    }}
                  >
                    <button
                      aria-label="Trim video in"
                      title="Trim in"
                      onPointerDown={e => {
                        e.stopPropagation();
                ...
                >
                  <label>
                    ...
                    <input
                      ...
                      value={start}
                      onChange={e =>
                        updateVideo(layer, { timeOffset: Math.max(0, Number(e.target.value) || 0) })
                      }
                      style={{ width: 60 }}
                    />
                  </label>
                      ...
                      onChange={e =>
                        updateVideo(layer, {
                          sourceIn: Math.max(
                            0,
                            Math.min(
                              (layer.sourceOut ?? duration ?? Number.POSITIVE_INFINITY) - 0.01,
                        ...
                        updateVideo(layer, {
                          sourceOut:
                            e.target.value === ""
                              ...
                              : Math.max(
                                  sourceIn + 0.01,
                                  Math.min(duration ?? Number.POSITIVE_INFINITY, Number(e.target.value) || 0),
                                ),
                        })
                      }
                      style={{ width: 55 }}
                  ...
                  <label>
                    ...
                    <input
                      ...
                      value={rate}
                      onChange={e =>
                        updateVideo(layer, { playbackRate: Math.max(0.01, Number(e.target.value) || 1) })
                      }
                      style={{ width: 55 }}
                    />
                  </label>
      ...
      <div style={{ marginTop: 12 }}>
        <b>Composition instances</b>
        {layers
          .filter(l => l.type === "composition" && l.compositionId)
          .map(layer => {
            const start = Math.max(0, layer.timeOffset ?? 0);
            const rate = layer.playbackRate && layer.playbackRate > 0 ? layer.playbackRate : 1;
            const sourceIn = layer.compositionIn ?? 0;
            const sourceOut = layer.compositionOut;
            const referenced = compositions.find(c => c.id === layer.compositionId);
            const sourceDuration = sourceOut ?? referenced?.duration ?? 10;
            const duration = Math.max(0, sourceDuration - sourceIn) / rate;
            const width = Math.max(0, Math.min(total - start, duration));
            return (
              <div key={layer.id} style={{ marginTop: 8 }}>
                <b>{layer.name ?? layer.compositionId}</b>
                <div
                  style={{
                    display: "flex",
                    gap: 6,
                ...
                >
                  <label>
                    ...
                    <input
                      ...
                      value={start}
                      onChange={e =>
                        updateInstance(layer, { timeOffset: Math.max(0, Number(e.target.value) || 0) })
                      }
                      style={{ width: 60 }}
                    />
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      value={rate}
                      onChange={e =>
                        updateInstance(layer, { playbackRate: Math.max(0.01, Number(e.target.value) || 1) })
                      }
                      style={{ width: 55 }}
                    />
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      value={sourceIn}
                      onChange={e =>
                        updateInstance(layer, { compositionIn: Math.max(0, Number(e.target.value) || 0) })
                      }
                      style={{ width: 55 }}
                    />
                  </label>
                      ...
                      onChange={e =>
                        updateInstance(layer, {
                          compositionOut:
                            e.target.value === ""
                              ? undefined
                              : Math.max(sourceIn, Number(e.target.value) || 0),
                        })
                      }
                      style={{ width: 55 }}
                    />
                  ...
                  <label>
                    <input
                      type="checkbox"
                      checked={!!layer.loop}
                      onChange={e => updateInstance(layer, { loop: e.target.checked })}
                    />{" "}
                    loop
                  </label>
                </div>
      ...
      {selected &&
        (() => {
          ...
          const k = t?.keyframes.find(x => x.id === selected.keyId);
          return t && k ? (
            <TimelineKeyframeEditor
              value={k}
              onTimeChange={time =>
                update(tracks.map(x => (x.id === t.id ? moveKeyframe(x, k.id, time) : x)))
              }
              onValueChange={value =>
                update(
                  tracks.map(x =>
                    x.id === t.id
                      ? { ...x, keyframes: x.keyframes.map(y => (y.id === k.id ? { ...y, value } : y)) }
                      : x,
                  ),
                )
              }
              ...
              onDelete={() => update(tracks.map(x => (x.id === t.id ? removeKeyframe(x, k.id) : x)))}
            />
          ) : null;
        })()}
    </section>
