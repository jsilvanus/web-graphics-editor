...
export function createHandleManager(
  scene: THREE.Scene,
    ...
    if (mode === "vertices") {
      for (let i = 0; i < data.geometry.vertices.length / 3; i++) {
        const handle = new THREE.Mesh(
          ...
          new THREE.MeshBasicMaterial({ color: selection.vertices.has(i) ? "#ffcc00" : "#fff" }),
        );
      ...
      for (const edge of edges) {
        const a = new THREE.Vector3().fromArray(data.geometry.vertices, edge.a * 3),
          b = new THREE.Vector3().fromArray(data.geometry.vertices, edge.b * 3);
        const line = new THREE.Line(
          new THREE.BufferGeometry().setFromPoints([a, b]),
          new THREE.LineBasicMaterial({
            color: selection.edges.has(edgeKey(edge.a, edge.b)) ? "#ffcc00" : "#fff",
            linewidth: 3,
          }),
        );
        line.userData.edgeKey = edgeKey(edge.a, edge.b);
        group.add(line);
      }
      ...
      for (const face of selection.faces) {
        const geometry = faceHandleGeometry(data, mesh, face);
        ...
        handle.userData.faceIndex = face;
        handle.renderOrder = 10;
        group.add(handle);
        const ids = geometry.getAttribute("position");
        for (let i = 0; i < ids.count; i++) {
          center.fromBufferAttribute(ids, i);
          count++;
        }
      }
      if (count) {
...
function averageVertices(data: Graphics3DMesh, ids: Set<number>) {
  ...
  ids.forEach(id => result.add(new THREE.Vector3().fromArray(data.geometry.vertices, id * 3)));
  return result.multiplyScalar(1 / ids.size);
}
function averageEdgeVertices(data: Graphics3DMesh, keys: Set<string>) {
  const result = new THREE.Vector3();
  let count = 0;
  for (const edge of meshEdges(data))
    if (keys.has(edgeKey(edge.a, edge.b))) {
      result.add(new THREE.Vector3().fromArray(data.geometry.vertices, edge.a * 3));
      result.add(new THREE.Vector3().fromArray(data.geometry.vertices, edge.b * 3));
      count += 2;
    }
