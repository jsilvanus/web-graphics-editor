...
export function CanvasLayerContent({
  ...
  if (layer.type === "composition" && renderNode)
    return (
      <CompositionLayerRenderer
        node={renderNode}
        selectedIds={selectedIds}
        worlds3d={worlds3d}
        views3d={views3d}
        currentTime={currentTime}
        onLayerPointerDown={onLayerPointerDown ?? (() => undefined)}
        onSelectLayer={onSelectLayer}
        onPathNodes={onNodes ? (id, nodes) => onNodes(nodes) : undefined}
  ...
  if (layer.type === "3d-view") {
    ...
    return view && world ? (
      <ThreeDViewLayer layer={layer} view={view} world={world} currentTime={currentTime} />
    ) : null;
  }
