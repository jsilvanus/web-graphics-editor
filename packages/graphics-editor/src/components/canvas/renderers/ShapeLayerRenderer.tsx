...
export function ShapeLayerRenderer({ layer }: { layer: Layer }) {
  const style: CSSProperties = { width: "100%", height: "100%", pointerEvents: "none", boxSizing: "border-box", background: styleValue(layer, "background", styleValue(layer, "fill", "transparent")), opacity: Number(layer.opacity ?? styleValue(layer, "opacity", 1)) };
  ...
  else { const radius = styleValue(layer, "border-radius", 0); style.borderRadius = typeof radius === "number" ? String(radius) + "px" : String(radius); }
