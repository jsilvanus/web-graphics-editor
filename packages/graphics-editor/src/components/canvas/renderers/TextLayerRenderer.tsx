...
function domToRuns(root: HTMLElement): TextRun[] {
  ...
  const visit = (node: Node, inherited: Omit<TextRun, "text"> = {}) => {
    ...
    if (style.textDecoration || node.tagName === "U" || node.tagName === "S" || node.tagName === "STRIKE") next.textDecoration = style.textDecoration || (node.tagName === "U" ? "underline" : "line-through");
...
export function TextLayerRenderer({ layer, layers = [], onTextCommit, onTextRunsCommit }: { layer: Layer; layers?: Layer[]; onTextCommit?: (text: string) => void; onTextRunsCommit?: (runs: TextRun[] | undefined) => void }) {
  ...
  const format = (command: "bold" | "italic" | "underline" | "strikeThrough") => { editorRef.current?.focus(); document.execCommand(command); setDraft(editorRef.current?.textContent ?? draft); };
  ...
  if (editing || !pathLayer) {
    if (!editing) return <div style={style} onDoubleClick={beginEdit}>{layer.textRuns?.length ? layer.textRuns.map((run, index) => <span key={index} style={{ fontFamily: run.fontFamily, fontSize: run.fontSize, fontWeight: run.fontWeight, fontStyle: run.fontStyle, color: run.color, letterSpacing: run.letterSpacing, textDecoration: run.textDecoration }}>{run.text}</span>) : layer.text}</div>;
    return <div style={{ position: "relative", width: "100%", height: "100%" }}>
      <div style={{ position: "absolute", top: 0, left: 0, zIndex: 2, display: "flex", gap: 3, padding: 3, background: "rgba(20,20,20,.9)", borderRadius: 4 }} onPointerDown={event => event.preventDefault()}><button type="button" title="Bold" onMouseDown={event => { event.preventDefault(); format("bold"); }}>B</button><button type="button" title="Italic" onMouseDown={event => { event.preventDefault(); format("italic"); }}>I</button><button type="button" title="Underline" onMouseDown={event => { event.preventDefault(); format("underline"); }}>U</button><button type="button" title="Strikethrough" onMouseDown={event => { event.preventDefault(); format("strikeThrough"); }}>S</button></div>
