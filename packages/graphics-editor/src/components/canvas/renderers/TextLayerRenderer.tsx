...
function domToRuns(root: HTMLElement): TextRun[] {
  ...
  const visit = (node: Node, inherited: Omit<TextRun, "text"> = {}) => {
    ...
    if (node.tagName === "B" || node.tagName === "STRONG" || style.fontWeight)
      next.fontWeight = style.fontWeight || "700";
    if (node.tagName === "I" || node.tagName === "EM" || style.fontStyle)
...
function normalizeRuns(runs: TextRun[]) {
  ...
  for (const run of runs) {
    ...
    const same =
      previous &&
      JSON.stringify({ ...previous, text: undefined }) === JSON.stringify({ ...run, text: undefined });
...
export function TextLayerRenderer({
  ...
  const pathLayer = textPathLayerId
    ? layers.find(
        candidate =>
          candidate.id === textPathLayerId && (candidate.type === "path" || candidate.type === "line"),
      )
    : undefined;
  ...
  const style: CSSProperties = {
    ...
    textDecoration: styleValue(layer, "text-decoration", "none"),
  ...
  useEffect(() => {
    ...
    editorRef.current.innerHTML = layer.textRuns?.length
      ? runsToHtml(layer.textRuns)
  ...
  const finishEditing = (commit: boolean) => {
    const value = commit ? (editorRef.current?.textContent ?? draft) : originalRef.current;
    const runs = commit && editorRef.current ? normalizeRuns(domToRuns(editorRef.current)) : undefined;
    setDraft(value);
    setEditing(false);
    if (commit) {
      if (value !== originalRef.current) onTextCommit?.(value);
      if (runs?.length) onTextRunsCommit?.(runs);
      else onTextRunsCommit?.(undefined);
    }
  };
  ...
  if (editing || !pathLayer) {
    if (!editing)
      return (
        <div style={style} onDoubleClick={beginEdit}>
          {layer.textRuns?.length
            ? layer.textRuns.map((run, index) => (
                <span
                  key={index}
                  style={{
    ...
    return (
      <div style={{ position: "relative", width: "100%", height: "100%" }}>
        ...
        <div
          ref={editorRef}
          style={{ ...style, outline: "1px dashed currentColor", paddingTop: 28 }}
          contentEditable
          role="textbox"
  ...
  const d =
    pathLayer.type === "line"
      ? pathDirection === "reverse"
        ? linePath(pathLayer.width, pathLayer.height, 0, 0)
        : linePath(0, 0, pathLayer.width, pathLayer.height)
      : nodes?.length
        ? nodesToD(nodes, pathLayer.closed)
        : pathLayer.path || (pathLayer.pathCommands ? pathCommandsToD(pathLayer.pathCommands) : "");
  ...
  return (
    ...
    >
      <defs>
        <path id={`${layer.id}-text-path`} d={d} transform={`translate(${tx} ${ty}) scale(${sx} ${sy})`} />
      </defs>
      <text
        textAnchor={anchor}
      ...
      >
        <textPath href={`#${layer.id}-text-path`} startOffset={`${offset}%`}>
          {runs.map((run, index) => (
            <tspan
              key={index}
              fontFamily={run.fontFamily}
