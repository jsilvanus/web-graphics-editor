...
export const PathEditor: FC<{ layer: Layer; onNodes: (nodes: PathNode[]) => void }> = ({ layer, onNodes }) => {
  ...
  const begin = (event: ReactPointerEvent, index: number, part: "node" | "in" | "out") => { const svg = event.currentTarget.ownerSVGElement; if (!svg) return; event.stopPropagation(); setSelected(previous => event.shiftKey ? (previous.includes(index) ? previous.filter(i => i !== index) : [...previous, index]) : [index]); const broken = event.altKey || event.metaKey; if (broken && part !== "node") { const next = clone(); next[index].kind = "corner"; onNodes(next); } const current = clone(); setDrag({ index, part, start: point(event, svg), nodes: current, broken }); (event.currentTarget as Element).setPointerCapture?.(event.pointerId); };
  const move = (event: ReactPointerEvent) => { if (!drag) return; const svg = event.currentTarget as SVGSVGElement; const p = point(event, svg); const dx = p.x - drag.start.x, dy = p.y - drag.start.y; const next = drag.nodes.map((node, i) => {
    if (!selected.includes(i) || i !== drag.index) return node;
  ...
  const deleteNode = (index: number) => { if (nodes.length <= 2) return; const next = clone(); next.splice(index, 1); onNodes(next); setSelected(previous => previous.filter(i => i !== index).map(i => i > index ? i - 1 : i)); };
  ...
  const keyDown = (event: ReactKeyboardEvent<SVGSVGElement>) => { if (!selected.length) return; if (event.key === "Delete" || event.key === "Backspace") { event.preventDefault(); selected.slice().sort((a,b) => b-a).forEach(deleteNode); } else if (event.key === "Enter") { event.preventDefault(); toggleKind(selected); } };
