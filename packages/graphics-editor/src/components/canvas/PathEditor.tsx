...
export const PathEditor: FC<{ layer: Layer; onNodes: (nodes: PathNode[]) => void }> = ({ layer, onNodes }) => {
  ...
  const begin = (event: ReactPointerEvent, index: number, part: "node" | "in" | "out") => { const svg = event.currentTarget.ownerSVGElement; if (!svg) return; event.stopPropagation(); setDrag({ index, part, start: point(event, svg), nodes: clone() }); };
  ...
  const toggleKind = (index: number) => { const next = clone(); const node = next[index]; node.kind = node.kind === "smooth" ? "corner" : "smooth"; onNodes(next); };
