...
function cloneNodes(nodes: PathNode[]) {
  return nodes.map(n => ({
    ...
    handleOut: n.handleOut && { ...n.handleOut },
  }));
}
...
export const PathEditor: FC<{ layer: Layer; onNodes: (nodes: PathNode[]) => void }> = ({
  layer,
  onNodes,
}) => {
  ...
  const point = (event: ReactPointerEvent, svg: SVGSVGElement): Point => {
    ...
    return {
      ...
      y: ((event.clientY - r.top) * layer.height) / r.height,
    };
  };
  ...
  return (
    <svg
      ...
      style={{ position: "absolute", inset: 0, overflow: "visible", zIndex: 500, pointerEvents: "none" }}
      tabIndex={0}
      aria-label="Path node editor"
      onKeyDown={keyDown}
      onPointerMove={move}
          ...
          <g key={i}>
            {nextIndex >= 0 && (
              <line
                ...
                onDoubleClick={e => insertNode(e, i)}
              />
            )}
            {node.handleIn && (
              <>
            ...
            <circle
              cx={node.x}
              cy={node.y}
              r={selected.includes(i) ? 9 : 7}
              fill={node.kind === "smooth" ? "#38bdf8" : "#fff"}
              stroke="#38bdf8"
              strokeWidth={selected.includes(i) ? 4 : 3}
              style={{ pointerEvents: "all", cursor: "move" }}
              onPointerDown={e => begin(e, i, "node")}
              onDoubleClick={() => toggleKind(i)}
              onContextMenu={e => {
                e.preventDefault();
                deleteNode(i);
              }}
            />
            {node.kind === "smooth" && !node.handleIn && (
              <circle
                cx={node.x - 11}
                cy={node.y}
                r="4"
            ...
            {node.kind === "smooth" && !node.handleOut && (
              <circle
                cx={node.x + 11}
                cy={node.y}
                r="4"
