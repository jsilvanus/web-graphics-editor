...
export const PathEditor: FC<{ layer: Layer; onNodes: (nodes: PathNode[]) => void }> = ({ layer, onNodes }) => {
  ...
  const begin = (event: ReactPointerEvent, index: number, part: "node" | "in" | "out") => { const svg = event.currentTarget.ownerSVGElement; if (!svg) return; event.stopPropagation(); setSelected(index); const broken = event.altKey || event.metaKey; if (broken && part !== "node") { const next = clone(); next[index].kind = "corner"; onNodes(next); } const current = clone(); setDrag({ index, part, start: point(event, svg), nodes: current, broken }); (event.currentTarget as Element).setPointerCapture?.(event.pointerId); };
  ...
  const toggleKind = (index: number) => { const next = clone(); next[index].kind = next[index].kind === "smooth" ? "corner" : "smooth"; onNodes(next); setSelected(index); };
  const addHandle = (index: number, part: "in" | "out") => { const next = clone(); const n = next[index]; const dx = part === "in" ? -50 : 50; const handle = { x: n.x + dx, y: n.y }; if (part === "in") n.handleIn = handle; else n.handleOut = handle; n.kind = "smooth"; onNodes(next); setSelected(index); };
  const deleteNode = (index: number) => { if (nodes.length <= 2) return; const next = clone(); next.splice(index, 1); onNodes(next); setSelected(selected === index ? null : selected !== null && selected > index ? selected - 1 : selected); };
  const insertNode = (event: ReactPointerEvent, index: number) => { const svg = event.currentTarget.ownerSVGElement; if (!svg) return; event.stopPropagation(); const p = point(event, svg); const next = clone(); next.splice(index + 1, 0, { x: p.x, y: p.y, kind: "corner" }); onNodes(next); setSelected(index + 1); };
  const keyDown = (event: ReactKeyboardEvent<SVGSVGElement>) => { if (selected === null) return; if (event.key === "Delete" || event.key === "Backspace") { event.preventDefault(); deleteNode(selected); } else if (event.key === "Enter") { event.preventDefault(); toggleKind(selected); } };
  return <svg width="100%" height="100%" viewBox={`0 0 ${Math.max(layer.width, 1)} ${Math.max(layer.height, 1)}`} style={{ position: "absolute", inset: 0, overflow: "visible", zIndex: 500, pointerEvents: "none" }} tabIndex={0} aria-label="Path node editor" onKeyDown={keyDown} onPointerMove={move} onPointerUp={() => setDrag(null)}>
    {nodes.map((node, i) => <g key={i}>
      ...
      <circle cx={node.x} cy={node.y} r={selected === i ? 9 : 7} fill={node.kind === "smooth" ? "#38bdf8" : "#fff"} stroke="#38bdf8" strokeWidth={selected === i ? 4 : 3} style={{ pointerEvents: "all", cursor: "move" }} onPointerDown={e => begin(e, i, "node")} onDoubleClick={() => toggleKind(i)} onContextMenu={e => { e.preventDefault(); deleteNode(i); }} />
