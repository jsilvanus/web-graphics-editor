...
export const PathEditor: FC<{ layer: Layer; onNodes: (nodes: PathNode[]) => void }> = ({ layer, onNodes }) => {
  ...
  const [drag, setDrag] = useState<{ index: number; part: "node" | "in" | "out"; start: Point; nodes: PathNode[]; selected: number[]; broken?: boolean } | null>(null);
  ...
  const keyDown = (event: ReactKeyboardEvent<SVGSVGElement>) => { if (!selected.length) return; if (event.key === "Delete" || event.key === "Backspace") { event.preventDefault(); selected.slice().sort((a,b) => b-a).forEach(deleteNode); } else if (event.key === "Enter") { event.preventDefault(); toggleKind(selected[0]); } };
