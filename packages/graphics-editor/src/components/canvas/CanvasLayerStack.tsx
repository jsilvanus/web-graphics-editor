...
export const CanvasLayerStack: FC<CanvasLayerStackProps> = ({ layers, selectedIds, worlds3d, views3d, currentTime = 0, onLayerPointerDown, onSelectLayer, onPathNodes, onTextCommit }) => (
  <>{getRootLayers(layers).map(layer => <CanvasLayer key={layer.id} layer={layer} layers={layers} selected={selectedIds.has(layer.id)} selectedIds={selectedIds} multiSelected={selectedIds.size > 1} worlds3d={worlds3d} views3d={views3d} currentTime={currentTime} onSelectLayer={onSelectLayer} onPointerDown={(event, kind, handle) => onLayerPointerDown(event, layer.id, kind, handle)} onLayerPointerDown={onLayerPointerDown} onNodes={nodes => onPathNodes?.(layer.id, nodes)} onTextCommit={onTextCommit} />)}</>
);

export { CanvasSelectionOverlay } from "./CanvasSelectionOverlay";
