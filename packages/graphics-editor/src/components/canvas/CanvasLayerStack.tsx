...
export const CanvasLayerStack: FC<CanvasLayerStackProps> = ({
  layers,
  frame,
  selectedIds,
  worlds3d,
  ...
  <>
    {getRootLayers(frame?.layers ?? layers).map(layer => (
      <CanvasLayer
        key={layer.id}
        layer={layer}
        renderNode={frame?.renderTree.find(node => node.layer.id === layer.id)}
        layers={frame?.layers ?? layers}
        selected={selectedIds.has(layer.id)}
        selectedIds={selectedIds}
        multiSelected={selectedIds.size > 1}
        worlds3d={worlds3d}
        views3d={frame?.views3d ?? views3d}
        videos={frame?.videos ?? []}
        assets={assets}
        currentTime={frame?.time ?? currentTime}
