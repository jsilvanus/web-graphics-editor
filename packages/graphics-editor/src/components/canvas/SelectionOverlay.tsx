...
export const SelectionOverlay: FC<{
  ...
  onPointerDown?: (event: ReactPointerEvent, kind: "resize" | "rotate", handle?: string) => void;
}> = ({ layers, selectedIds, onPointerDown }) => {
  const selected = layers.filter(l => selectedIds.has(l.id));
  if (!selected.length) return null;
  const boxes = selected.map(l => ({ x: l.x, y: l.y, r: l.x + l.width, b: l.y + l.height }));
