...
export function ThreeDViewLayer({ layer, view, world, currentTime }: ThreeDViewLayerProps) {
  ...
  const settings = view.renderSettings ?? {};
  const output = get3DOutputSize(layer.width, layer.height, settings);
  const basePixelRatio = Math.min(window.devicePixelRatio || 1, settings.maxPixelRatio ?? 2);
  const pixelRatio = Math.max(0.25, basePixelRatio);
  const renderOptions = {
    pixelRatio,
    width: output.width,
    ...
    environmentColor: settings.environmentColor,
    environmentIntensity: settings.environmentIntensity,
  };
  const renderWorld = {
    ...world,
    meshes: mappedWorld.meshes,
    cameras: mappedWorld.cameras,
    lights: mappedWorld.lights,
  };
  const renderKey = camera
    ? `${create3DRenderCacheKey(renderWorld, camera, view, output.width, output.height, pixelRatio)}@${mappedWorld.worldTime}`
    : "";
  useEffect(() => {
  ...
  }, [live, camera?.id, layer.id]);
  useEffect(() => {
    if (!camera) return;
    if (live) {
      rendererRef.current?.render(renderWorld, camera, view, renderOptions);
      return;
    }
    const cached = get3DRenderCache(renderKey);
    if (cached) {
      setCachedImage(cached.image);
      return;
    }
    const host = document.createElement("div"),
      renderer = new ThreeGraphics3DRenderer();
    renderer.mount(host, true);
    renderer.render(renderWorld, camera, view, renderOptions);
    const canvas = renderer.getCanvas();
    if (canvas) {
      const image = canvas.toDataURL("image/png");
      set3DRenderCache(renderKey, image);
  ...
  const opacity = Math.max(0, Math.min(1, Number(layer.style?.opacity ?? 1)));
  ...
  return (
    <div ref={hostRef} id={`ge-3d-live-${layer.id}`} style={style}>
      {!live && cachedImage && (
        <img
          src={cachedImage}
          alt=""
