...
/** Renders a stored 3D camera view inside the ordinary transparent 2D composition. */
export function ThreeDViewLayer({ layer, view, world }: ThreeDViewLayerProps) {
  ...
  const style: React.CSSProperties = { width: "100%", height: "100%", overflow: "hidden", pointerEvents: "none", opacity };
  if (!live) return <div style={style}>{cachedImage && <img src={cachedImage} alt="" draggable={false} style={{ width: "100%", height: "100%", objectFit: "fill", display: "block" }} />}</div>;
  return <div id={`ge-3d-live-${layer.id}`} style={style} />;
}
