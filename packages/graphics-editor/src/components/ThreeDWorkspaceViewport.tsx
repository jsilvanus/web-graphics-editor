...
export function ThreeDWorkspaceViewport({ world, cameraId, selectedId, mode, onSelect, onChange }: ThreeDWorkspaceViewportProps) {
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      for (const mesh of current.meshes) {
        ...
        if (!object) {
          object = new THREE.Mesh(createThreeGeometry(mesh), new THREE.MeshStandardMaterial({ color: mesh.material?.color ?? "#78a9ff", roughness: mesh.material?.roughness ?? 0.75, metalness: mesh.material?.metalness ?? 0, wireframe: mesh.material?.wireframe ?? false }));
      ...
      for (const [id, object] of objectsRef.current) if (!current.meshes.some(mesh => mesh.id === id)) {
        scene.remove(object); object.geometry.dispose(); (object.material as THREE.Material).dispose(); objectsRef.current.delete(id);
      }
    ...
    const onObjectChange = () => {
      ...
      const object = id ? objectsRef.current.get(id) : undefined;
    ...
    const resize = () => { const width = host.clientWidth || 1, height = host.clientHeight || 1; renderer.setSize(width, height, false); if (camera instanceof THREE.PerspectiveCamera) camera.aspect = width / height; camera.updateProjectionMatrix(); };
  ...
  return <div ref={hostRef} style={{ minHeight: 520, position: "relative" }}><div style={{ position: "absolute", top: 10, left: 10, zIndex: 2, display: "flex", gap: 6 }}><button onClick={() => transformRef.current?.setMode("translate")}>Move</button><button onClick={() => transformRef.current?.setMode("rotate")}>Rotate</button><button onClick={() => transformRef.current?.setMode("scale")}>Scale</button></div></div>;
}
