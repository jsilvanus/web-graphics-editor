...
export interface DrawingPreview {
  tool: "line" | "path" | "orthogonal" | "polygon" | "star" | "freehand";
  ...
  return (
    <div className="ge-canvas-wrap">
      ...
      >
        ...
        <div
          className="ge-viewport-controls"
          style={{ position: "absolute", right: 8, top: 8, zIndex: 1000, display: "flex", gap: 4 }}
        >
          <button onClick={onZoomOut} type="button" title="Zoom out">
            −
          </button>
          <button onClick={onZoomReset} type="button" title="100%">
            100%
          </button>
          <span style={{ padding: "4px 7px", background: "#222", color: "#fff", fontSize: 12 }}>
            {Math.round(zoom * 100)}%
          </span>
          <button onClick={onZoomIn} type="button" title="Zoom in">
            +
          </button>
          <button onClick={onFit} type="button" title="Fit canvas">
            Fit
          </button>
        </div>
        <div
        ...
        >
          ...
          >
            ...
            {safe && (
              <>
                <div className="ge-safe safe90" />
                <div className="ge-safe safe80" />
              </>
            )}
          </div>
        </div>
        <div
          className="ge-canvas-status"
          aria-live="polite"
          style={{
            position: "absolute",
            left: 36,
        ...
        >
          {cursor ? `X ${Math.round(cursor.x)}  Y ${Math.round(cursor.y)}` : "Move over canvas"} ·{" "}
          {Math.round(zoom * 100)}%
        </div>
      </div>
    </div>
