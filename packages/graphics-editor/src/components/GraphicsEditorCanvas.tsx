...
export const GraphicsEditorCanvas: FC<{
  ...
  onLayerPointerDown, onPathNodes, drawing, onDrawingDoubleClick,
  worlds3d = [], views3d = [],
}) => (
  <div className="ge-canvas-wrap">
    ...
    >
      <div className="ge-viewport-controls" style={{ position: "absolute", right: 8, top: 8, zIndex: 1000, display: "flex", gap: 4 }}>
        ...
        <span style={{ padding: "4px 7px", background: "#222", color: "#fff", fontSize: 12 }}>{Math.round(zoom * 100)}%</span>
        <button onClick={onZoomIn} type="button">+</button>
        <button onClick={onFit} type="button">Fit</button>
      </div>
      <div className="ge-canvas" onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerLeave={onPointerUp}>
        <div
          ...
          style={{ background, transform: `translate(${panX}px,${panY}px) scale(${zoom})`, transformOrigin: "0 0" }}
        >
          {grid && <div className="ge-grid" />}
          <div className="ge-guide ge-guide-v" />
          <div className="ge-guide ge-guide-h" />
          <CanvasLayerStack layers={layers} selectedIds={selectedIds} worlds3d={worlds3d} views3d={views3d} onLayerPointerDown={onLayerPointerDown} onPathNodes={onPathNodes} />
          {drawing && drawing.points.length > 0 && (
            <svg className="ge-drawing-preview" width="100%" height="100%" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 200 }} onDoubleClick={onDrawingDoubleClick}>
