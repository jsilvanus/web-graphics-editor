...
export function alignLayers(
  ...
  const b =
    reference === "canvas" ? { left: 0, top: 0, right: canvasWidth, bottom: canvasHeight } : bounds(selected);
  return layers.map(layer => {
    if (!ids.has(layer.id)) return layer;
    switch (mode) {
      case "left":
        return { ...layer, x: b.left };
      case "center-x":
        return { ...layer, x: b.left + (b.right - b.left - layer.width) / 2 };
      case "right":
        return { ...layer, x: b.right - layer.width };
      case "top":
        return { ...layer, y: b.top };
      case "center-y":
        return { ...layer, y: b.top + (b.bottom - b.top - layer.height) / 2 };
      case "bottom":
        return { ...layer, y: b.bottom - layer.height };
    }
  });
}
