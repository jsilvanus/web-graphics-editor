...
describe("layer tree", () => {
  it("returns only root layers", () =>
    expect(getRootLayers(layers).map(x => x.id)).toEqual(["group", "root"]));
  it("resolves children in declared order", () =>
    expect(getChildLayers(layers, "group").map(x => x.id)).toEqual(["a", "nested"]));
  it("collects descendants recursively", () =>
    expect([...getDescendantIds(layers, "group")]).toEqual(["group", "a", "nested", "b"]));
  it("computes subtree bounds", () =>
    expect(getLayerTreeBounds(layers, "group")).toEqual({ x: 10, y: 20, width: 110, height: 40 }));
  it("handles missing ids safely", () =>
    expect(getLayerTreeBounds(layers, "missing")).toEqual({ x: 0, y: 0, width: 0, height: 0 }));
});
