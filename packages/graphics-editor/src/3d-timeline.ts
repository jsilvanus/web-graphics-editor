...
export function add3DTrack(
  ...
  return {
    ...world,
    timeline: { ...(world.timeline ?? {}), tracks: [...(world.timeline?.tracks ?? []), track] },
  };
}
export function remove3DTrack(world: Graphics3DWorld, trackId: string): Graphics3DWorld {
  return {
    ...
    timeline: {
      ...(world.timeline ?? {}),
      tracks: (world.timeline?.tracks ?? []).filter(track => track.id !== trackId),
    },
  };
}
...
export function upsert3DKeyframe(
  ...
  return {
    ...
    timeline: {
      ...(world.timeline ?? {}),
      tracks: (world.timeline?.tracks ?? []).map(track =>
        track.id !== trackId
          ? track
          : {
              ...track,
              keyframes: [
                ...track.keyframes.filter(
                  k => k.id !== keyframe.id && Math.abs(k.time - keyframe.time) > 0.0001,
                ),
                keyframe,
...
export function move3DKeyframe(
  ...
  const track = (world.timeline?.tracks ?? []).find(t => t.id === trackId),
    key = track?.keyframes.find(k => k.id === keyframeId);
  return key ? upsert3DKeyframe(world, trackId, { ...key, time: Math.max(0, time) }) : world;
}
