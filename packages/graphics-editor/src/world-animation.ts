import type { AnimationKeyframe, AnimationValue, Graphics3DWorld, Graphics3DTrack, Graphics3DMesh, Graphics3DCamera } from "./types";
...
export function evaluateWorldAtTime(world:Graphics3DWorld,wegraTime:number,mapping:Parameters<typeof mapWorldTime>[1]):Evaluated3DWorld{
 ...
 for(const track of tracks(world)){
  ...
  else cameras=cameras.map(c=>c.id===track.targetId?applyCameraTrack(c,track.property,value):c);
 }
