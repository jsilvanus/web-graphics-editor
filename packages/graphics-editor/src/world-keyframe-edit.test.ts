...
describe("editWorldPropertyAtTime", () => {
  it("creates a track and keyframe when editing an unanimated property", () => {
    ...
    expect(r.world.timeline?.tracks).toHaveLength(1);
  ...
  it("updates the existing keyframe instead of creating another", () => {
    ...
    expect(b.world.timeline?.tracks[0].keyframes).toHaveLength(1);
    expect(b.world.timeline?.tracks[0].keyframes[0].value).toBe(120);
  });
