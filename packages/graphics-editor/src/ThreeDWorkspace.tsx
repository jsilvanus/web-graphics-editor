...
export function ThreeDWorkspace({ world: inputWorld, onChange, className }: ThreeDWorkspaceProps) {
  ...
  const addBox = useCallback(() => { const id = `box-${Date.now()}`; onChange({ ...world, meshes: [...world.meshes, createBoxMesh(id, 2, 2, 2, { position: [0, 1, 0], rotation: [0, 0, 0], scale: [1, 1, 1] })] }); setSelectedId(id); }, [onChange, world]);
  ...
  const addCamera = useCallback(() => { const id = `camera-${Date.now()}`; onChange({ ...world, cameras: [...world.cameras, { ...DEFAULT_CAMERA, id, name: `Camera ${world.cameras.length + 1}` }] }); setCameraId(id); }, [onChange, world]);
  ...
  return <div className={className} style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 260px", minHeight: 520, background: "#101216", color: "#eee", border: "1px solid #30343b", borderRadius: 8, overflow: "hidden" }}>
    <ThreeDWorkspaceViewport world={world} cameraId={cameraId} selectedId={selectedId} mode={mode} onModeChange={setMode} onSelect={setSelectedId} onChange={onChange} />
