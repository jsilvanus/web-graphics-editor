...
export function ThreeDWorkspace({ world: inputWorld, onChange, className }: ThreeDWorkspaceProps) {
  ...
  const addBox = useCallback(() => { const id = `box-${Date.now()}`; onChange({ ...world, meshes: [...world.meshes, createBoxMesh(id, 2, 2, 2, { position: [0, 1, 0], rotation: [0, 0, 0], scale: [1, 1, 1] })] }); setSelectedId(id); setEditCamera(false); }, [onChange, world]);
  ...
  const addCamera = useCallback(() => { const id = `camera-${Date.now()}`; onChange({ ...world, cameras: [...world.cameras, { ...DEFAULT_CAMERA, id, name: `Camera ${world.cameras.length + 1}` }] }); setCameraId(id); setEditCamera(true); }, [onChange, world]);
  const addLight = useCallback(() => { const light: Graphics3DLight = { id: `light-${Date.now()}`, type: "directional", position: [4, 6, 4], intensity: 2 }; onChange({ ...world, lights: [...(world.lights ?? []), light] }); }, [onChange, world]);
  return <div className={className} style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 260px", minHeight: 520, background: "#101216", color: "#eee", border: "1px solid #30343b", borderRadius: 8, overflow: "hidden" }}>
    <ThreeDWorkspaceViewport world={world} cameraId={cameraId} selectedId={selectedId} mode={mode} editCamera={editCamera} onModeChange={setMode} onSelect={setSelectedId} onChange={onChange} />
    <ThreeDWorkspaceInspector world={world} selectedId={selectedId} cameraId={cameraId} editCamera={editCamera} onEditCamera={setEditCamera} onSelect={setSelectedId} onUpdateMesh={updateMesh} onUpdateCamera={updateCamera} onAddBox={addBox} onDeleteMesh={deleteMesh} onCameraChange={id => { setCameraId(id); setEditCamera(true); }} onAddCamera={addCamera} onAddLight={addLight} />
  </div>;
}
