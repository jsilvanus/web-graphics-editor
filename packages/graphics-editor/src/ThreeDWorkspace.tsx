...
export function ThreeDWorkspace({ world: inputWorld, onChange, className }: ThreeDWorkspaceProps) {
  ...
  const updateMesh = useCallback(
    (id: string, patch: Partial<Omit<Graphics3DMesh, "id">>) => onChange(updateWorldMesh(world, id, patch)),
    [onChange, world],
  );
  const updateCamera = useCallback(
    (id: string, patch: Partial<Omit<Graphics3DCamera, "id">>) =>
      onChange(updateWorldCamera(world, id, patch)),
    [onChange, world],
  );
  const updateLight = useCallback(
    (id: string, patch: Partial<Omit<Graphics3DLight, "id">>) =>
      onChange({
        ...world,
        lights: (world.lights ?? []).map(light => (light.id === id ? { ...light, ...patch } : light)),
      }),
    [onChange, world],
  );
  const addBox = useCallback(() => {
    ...
    onChange({
      ...
      meshes: [
        ...world.meshes,
        createBoxMesh(id, 2, 2, 2, { position: [0, 1, 0], rotation: [0, 0, 0], scale: [1, 1, 1] }),
      ],
    });
    setSelectedId(id);
  ...
  const addCamera = useCallback(() => {
    ...
    onChange({
      ...world,
      cameras: [...world.cameras, { ...DEFAULT_CAMERA, id, name: `Camera ${world.cameras.length + 1}` }],
    });
    setCameraId(id);
    setEditCamera(true);
    setSelectedLightId(null);
  ...
  const addLight = useCallback(() => {
    ...
    const light: Graphics3DLight = { id, type: "directional", position: [4, 6, 4], intensity: 2 };
  ...
  return (
    ...
    >
      <ThreeDWorkspaceViewport
        ...
        onChange={onChange}
      />
      <ThreeDWorkspaceInspector
        world={world}
        selectedId={selectedId}
        selectedLightId={selectedLightId}
        cameraId={cameraId}
        editCamera={editCamera}
        onEditCamera={setEditCamera}
        onSelect={id => {
          setSelectedId(id);
          setSelectedLightId(null);
        }}
