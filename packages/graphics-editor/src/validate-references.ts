...
const ids = <T extends { id: string }>(items: T[] | undefined): Map<string, T> =>
...
export function validateDocumentReferences(document: GraphicsDocument): DocumentValidationResult {
  ...
  for (const layer of document.layers) {
    ...
    for (const childId of layer.children ?? [])
      requireReference(layers, childId, `layer "${layer.id}" children`, errors);
    ...
    if (fontAssetId) {
      ...
      if (asset && asset.type !== "font")
        errors.push(`layer "${layer.id}" fontAssetId "${fontAssetId}" is not a font asset`);
    }
  }

  for (const composition of document.compositions ?? []) {
    for (const layerId of composition.layerIds)
      requireReference(layers, layerId, `composition "${composition.id}" layerIds`, errors);
  }

  for (const viewport of document.viewports ?? []) {
    for (const compositionId of viewport.compositionIds ?? [])
      requireReference(compositions, compositionId, `viewport "${viewport.id}" compositionIds`, errors);
  }
  ...
  if (timeline) {
    ...
    for (const clip of timeline.clips ?? [])
      requireReference(layers, clip.layerId, `timeline clip "${clip.id}" layerId`, errors);
    if (timeline.currentSceneId)
      requireReference(
        new Map(timeline.scenes.map(scene => [scene.id, scene])),
  ...
  for (const view of document.views3d ?? []) {
    ...
    if (world)
      requireReference(
        new Map(world.cameras.map(camera => [camera.id, camera])),
        view.cameraId,
        `3D view "${view.id}" cameraId`,
        errors,
      );
  }
  ...
  for (const world of document.worlds3d ?? []) {
    ...
    for (const track of world.timeline?.tracks ?? []) {
      ...
      requireReference(
        targetMap,
        track.targetId,
        `world "${world.id}" ${track.targetType} track "${track.id}" targetId`,
        errors,
      );
    }
    for (const mesh of world.meshes) {
      ...
      if (textureAssetId) {
        requireReference(
          assets,
          textureAssetId,
          `world "${world.id}" mesh "${mesh.id}" textureAssetId`,
          errors,
        );
        const asset = document.assets?.find(item => item.id === textureAssetId);
        if (asset && asset.type !== "image" && asset.type !== "video")
          errors.push(
            `world "${world.id}" mesh "${mesh.id}" textureAssetId "${textureAssetId}" is not an image or video asset`,
          );
      }
    }
  }
  ...
  for (const output of document.outputs ?? []) {
    if (output.viewportId)
      requireReference(viewports, output.viewportId, `output "${output.id}" viewportId`, errors);
  }
