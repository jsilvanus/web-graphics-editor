...
function fallback(a:PolygonPoint[],b:PolygonPoint[],op:BooleanOperation){const ai=inside(b[0],a),bi=inside(a[0],b);if(op==="intersect")return ai?[b]:bi?[a]:[];if(op==="union")return ai?[a]:bi?[b]:[a,b];return ai?[]:bi?[a]:[a,b]}
...
export function pathNodesToPolygon(nodes:PathNode[]):PolygonPoint[]|null{
 if(nodes.length<3||nodes.some(n=>n.handleIn||n.handleOut))return null;
