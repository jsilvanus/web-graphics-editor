...
describe("document operations", () => {
  ...
  it("updates a layer style without replacing other style values", () => {
    const next = updateLayerStyle(
      { ...doc, layers: [{ ...doc.layers[0], style: { color: "white" } }] },
      "a",
      "font-size",
      "48px",
    );
  ...
  it("moves a layer one step forward", () =>
    expect(ids(bringLayerForward(doc, "b"))).toEqual(["a", "c", "b", "d"]));
  it("moves a layer one step backward", () =>
    expect(ids(sendLayerBackward(doc, "c"))).toEqual(["a", "c", "b", "d"]));
  it("moves a layer to the front", () =>
    expect(ids(bringLayerToFront(doc, "b"))).toEqual(["a", "c", "d", "b"]));
  ...
  it("moves a group and all descendants by the same delta", () => {
    const grouped: GraphicsDocument = {
      ...doc,
      layers: [
