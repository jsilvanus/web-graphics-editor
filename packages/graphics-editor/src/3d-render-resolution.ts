...
export interface Graphics3DOutputSize {
  width: number;
  height: number;
}
 ...
 */
export function get3DOutputSize(
  layerWidth: number,
  layerHeight: number,
  settings?: Graphics3DRenderSettings,
): Graphics3DOutputSize {
  ...
  return {
    width: Math.max(1, Math.round(layerWidth * scale)),
    height: Math.max(1, Math.round(layerHeight * scale)),
  };
}
