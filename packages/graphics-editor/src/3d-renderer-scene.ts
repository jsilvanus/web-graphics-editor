...
function addLight(scene: THREE.Scene, light: Graphics3DLight, shadows: boolean): void {
...
export function createThreeScene(
  ...
  for (const light of world.lights ?? []) addLight(scene, light, shadows);
  if (options.environmentColor)
    scene.add(
      new THREE.HemisphereLight(options.environmentColor, "#111111", options.environmentIntensity ?? 0.35),
    );
