...
function sortJsonValue(value: unknown): unknown {
  ...
  if (value && typeof value === "object") {
    ...
    return Object.fromEntries(
      Object.keys(input)
        .sort()
        .map(key => [key, sortJsonValue(input[key])]),
    );
  }
  return value;
}
...
function migrateV1ToV2(value: unknown): unknown {
  ...
  if (manifest.format !== WEGRA_FORMAT || manifest.version !== 1)
    throw new Error("Invalid WEGRA v1 manifest");
...
export function migrateWegraManifest(value: unknown): WegraManifest {
  ...
  if (current.format !== WEGRA_FORMAT || typeof version !== "number" || !Number.isInteger(version))
    throw new Error("Invalid WEGRA manifest");
  ...
  const assets = Array.isArray(current.assets)
    ? current.assets.filter((x): x is string => typeof x === "string").sort()
    : [];
  return {
    format: WEGRA_FORMAT,
...
async function assetBytes(asset: GraphicsAsset) {
  if (asset.url.startsWith("data:")) {
    const comma = asset.url.indexOf(","),
      meta = asset.url.slice(5, comma),
      body = asset.url.slice(comma + 1);
...
export async function serializeWegra(project: WegraProject): Promise<Uint8Array> {
  ...
  for (const asset of assets) {
    try {
      const data = await assetBytes(asset),
  ...
  const manifest: WegraManifest = {
    ...
    provenance: "provenance/actors.json",
    assets: embedded
      .filter(a => a.url.startsWith("assets/"))
      .map(a => a.url)
      .sort(),
  };
  files.unshift(
  ...
  if (project.history)
    files.push({ name: "history/history.json", data: encoder.encode(canonicalJson(project.history)) });
  return zip(files);
}
...
export function deserializeWegra(bytes: Uint8Array): WegraProject {
  ...
  while (p + 4 <= bytes.length) {
    ...
    if (sig === 0x04034b50) {
      const nameLen = view.getUint16(p + 26, true),
        ...
        name = decoder.decode(bytes.slice(p + 30, p + 30 + nameLen)),
  ...
  return {
    document,
    actors: JSON.parse(
      decoder.decode(files["provenance/actors.json"] ?? encoder.encode('{"actors":{}}')),
    ) as ActorVocabulary,
    history:
      manifest.history && files["history/history.json"]
        ? (JSON.parse(decoder.decode(files["history/history.json"])) as DocumentHistory)
        : undefined,
  };
}
