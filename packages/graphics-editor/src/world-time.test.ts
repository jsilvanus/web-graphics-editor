...
describe("world time mapping", () => {
  ...
  it("loops inside the configured world range", () => {
    ...
    expect(mapWorldTime(0, m)).toBe(2);
    expect(mapWorldTime(1, m)).toBe(3);
    expect(mapWorldTime(4, m)).toBe(2);
    expect(mapWorldTime(5, m)).toBe(3);
  });
  it("loops correctly for negative mapped time", () =>
    expect(mapWorldTime(-1, { offset: 0, rate: 1, loop: true, inPoint: 2, outPoint: 6 })).toBe(5));
