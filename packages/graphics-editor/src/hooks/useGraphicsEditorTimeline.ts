...
export function createDefaultTimeline(): SceneTimeline {
  ...
  return { scenes: [scene], currentSceneId: scene.id, currentTime: 0, tracks: [], clips: [] };
}
export function createDefaultWorldTimeline(): Graphics3DWorldTimeline {
...
export function useGraphicsEditorTimeline(
  ...
  const enterWorld = useCallback((worldId: string) => {
    setContext({ kind: "world", worldId });
    setWorldCurrentTime(0);
  }, []);
  const exitWorld = useCallback(() => setContext({ kind: "main" }), []);
  const seek = useCallback(
    (time: number) => {
      if (context.kind === "world") {
        const max = worldTimeline.duration ?? 0;
        setWorldCurrentTime(Math.max(0, Math.min(max, time)));
        return;
      }
      const next = {
        ...mainTimeline,
  ...
  const updateWorldTimeline = useCallback(
    (next: Graphics3DWorldTimeline) => {
      if (context.kind !== "world") return;
      executeCommand(
        {
          document: {
            ...document,
            worlds3d: (document.worlds3d ?? []).map(w =>
              w.id === context.worldId ? { ...w, timeline: next } : w,
            ),
          },
        },
        { label: "Update world timeline" },
      );
    },
    [context, document, executeCommand],
  );
