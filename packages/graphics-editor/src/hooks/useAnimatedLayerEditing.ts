...
type ExecuteCommand = (
  command: { document: GraphicsDocument; operation?: DocumentOperation },
...
export function useAnimatedLayerEditing(
  ...
  const changeLayer = useCallback(
    (id: string, patch: Partial<Layer>) => {
      ...
      for (const [property, rawValue] of Object.entries(patch)) {
        if (
          ANIMATED_PROPERTIES.includes(property as AnimatedProperty) &&
          typeof rawValue === "number" &&
          current.timeline?.tracks.some(track => track.layerId === id && track.property === property)
        ) {
          const result = setAnimatedPropertyAtTime(
            current,
            id,
            property as AnimatedProperty,
            currentTime,
            rawValue,
          );
          if (result.operation) {
            current = result.document;
            executeCommand(result, { label: `Set ${property} at ${currentTime.toFixed(2)}s` });
          }
        } else {
          const result = updateLayerCommand(current, id, { [property]: rawValue } as Partial<Layer>);
          if (result.operation) {
            current = result.document;
            executeCommand(result, { label: `Set ${property}` });
          }
        }
      }
    },
  ...
  const changeStyle = useCallback(
    (id: string, key: string, value: string | number) => {
      const trackProperty =
        key === "text-path-start-offset" ? "textPathStartOffset" : key === "opacity" ? "opacity" : undefined;
      if (
        trackProperty &&
        typeof value === "number" &&
        document.timeline?.tracks.some(track => track.layerId === id && track.property === trackProperty)
      ) {
        const result = setAnimatedPropertyAtTime(
          document,
          id,
          trackProperty as AnimatedProperty,
          currentTime,
          value,
        );
