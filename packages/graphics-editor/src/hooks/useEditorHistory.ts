...
export function useEditorHistory(initialDocument: GraphicsDocument) {
  ...
  const setDocument = useCallback((next: GraphicsDocument | ((current: GraphicsDocument) => GraphicsDocument), history = true) => {
    ...
    if (history) { pastRef.current = [...pastRef.current.slice(-99), current]; futureRef.current = []; }
    apply(resolved); forceUpdate(value => value + 1);
  }, [apply]);
  ...
  const recordOperation = useCallback((operation: DocumentOperation, options: EditorOperationOptions = {}) => {
    ...
    apply(next); forceUpdate(value => value + 1);
    return next;
  }, [apply]);

  const execute = recordOperation;

  const undo = useCallback(() => { const previous = pastRef.current.pop(); if (!previous) return; futureRef.current.push(documentRef.current); apply(previous); forceUpdate(value => value + 1); }, [apply]);
  const redo = useCallback(() => { const next = futureRef.current.pop(); if (!next) return; pastRef.current.push(documentRef.current); apply(next); forceUpdate(value => value + 1); }, [apply]);
  const resetHistory = useCallback((next: GraphicsDocument) => { pastRef.current = []; futureRef.current = []; historyRef.current = createHistory(next); apply(next); forceUpdate(value => value + 1); }, [apply]);
