...
export function useEditorHistory(initialDocument: GraphicsDocument) {
  ...
  const recordOperation = useCallback(
    (operation: DocumentOperation, options: EditorOperationOptions = {}) => {
  ...
  const executeCommand = useCallback(
    (command: DocumentCommand, options: EditorOperationOptions = {}) => {
  ...
  const setDocument = useCallback(
    (
      next: GraphicsDocument | ((current: GraphicsDocument) => GraphicsDocument),
      history = true,
      ...
      const operations = diffOperations(current, resolved);
      if (!operations.length) return;
      const operation = operations.length === 1 ? operations[0] : { type: "batch" as const, operations };
      executeCommand({ document: resolved, operation }, options);
    },
