...
export function useLayerOperations(setDocument: (next: GraphicsDocument | ((current: GraphicsDocument) => GraphicsDocument), history?: boolean) => void) {
  ...
  const updateStyle = useCallback((id: string, key: string, value: string | number) => setDocument(document => ({ ...document, layers: document.layers.map(layer => { if (layer.id !== id) return layer; const style = { ...(layer.style ?? {}) }; if (value === "") delete style[key]; else style[key] = value; return { ...layer, style }; }) })), [setDocument]);
