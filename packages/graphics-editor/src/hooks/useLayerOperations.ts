...
function createLayer(type: LayerType): Layer {
  ...
  return {
    ...
    y: 300,
    width: 1000,
    height: type === "ellipse" ? 440 : 500,
    style: { background: type === "ellipse" ? "#fff" : "#111" },
  };
}
    ...
    (ids: Set<string>) => {
      ...
      if (operations.length)
        execute(
          {
            document: next,
            operation: operations.length === 1 ? operations[0] : { type: "batch", operations },
          },
          "Remove layers",
        );
    },
  ...
  const duplicate = useCallback(
    (ids: Set<string>) => {
      ...
      const copies = selected.map(layer => ({
        ...
        pathCommands: layer.pathCommands?.map(command => ({ ...command })),
        nodes: layer.nodes?.map(node => ({
          ...node,
          handleIn: node.handleIn && { ...node.handleIn },
