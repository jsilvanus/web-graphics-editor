...
export function useCanvasInteraction(document:GraphicsDocument,artboardRef:RefObject<HTMLDivElement|null>,grid:boolean,aspectLock:boolean,onChange:(next:GraphicsDocument)=>void,selectedIds:Set<string>=new Set(),referenceLayer:Layer|null=null){
 ...
 const pointerMove=useCallback((event:ReactPointerEvent)=>{const d=dragRef.current;if(!d)return;const dx=(event.clientX-d.startX)/d.scale,dy=(event.clientY-d.startY)/d.scale;let next=d.layers.map(l=>({...l}));
  ...
  else if(d.kind==="resize"){const h=d.handle??"se",start=bounds(d.layers),alt=event.altKey;let sx=1,sy=1,ax=d.centerX,ay=d.centerY;if(h.includes("e")){sx=(start.width+dx)/start.width;ax=alt?d.centerX:start.x}else if(h.includes("w")){sx=(start.width-dx)/start.width;ax=alt?d.centerX:start.x+start.width}if(h.includes("s")){sy=(start.height+dy)/start.height;ay=alt?d.centerY:start.y}else if(h.includes("n")){sy=(start.height-dy)/start.height;ay=alt?d.centerY:start.y+start.height}if(event.shiftKey||aspectLock){const s=Math.max(.05,Math.abs(Math.abs(sx-1)>Math.abs(sy-1)?sx:sy));sx=(sx<0?-1:1)*s;sy=(sy<0?-1:1)*s}next=next.map(l=>{const x=Math.round(ax+(l.x-ax)*sx),y=Math.round(ay+(l.y-ay)*sy);return{...l,x,y,width:Math.max(20,Math.round(l.width*Math.abs(sx))),height:Math.max(20,Math.round(l.height*Math.abs(sy)))}});}
 ...
 },[document,grid,aspectLock,onChange,referenceLayer]);
